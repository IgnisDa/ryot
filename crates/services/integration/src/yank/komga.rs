    ...
    impl Metadata {
        fn extract_id(&self, url: &str, label: &str) -> Option<String> {
            ...
            match label_lower.as_str() {
                label if label.contains("google") => from_query_param(),
                label if label.contains("mangaupdates") => {
                    from_query_param().or_else(from_path_segment)
                }
                _ => from_path_segment(),
            }
        }
...
pub async fn yank_progress(
    ...
    let books: komga_book::Response = client
        .post(format!(
            "{url}/books/list?sort=readProgress.lastModified,desc&unpaged=true"
        ))
    ...
    let unique_collection_updates: HashMap<String, _> = stream::iter(item.content)
        .filter_map(|book| async move {
            match find_provider_and_id(ss, &book).await {
                ...
                _ => {
                    ryot_log!(debug, "No URL or database entry found: {}", book.name);
                    None
                }
            }
        })
