...
use dependent_utils::{get_entity_title_from_id_and_lot, send_notification_for_user};
...
pub async fn perform_background_jobs(service: &crate::MiscellaneousService) -> Result<()> {
    ryot_log!(debug, "Starting background jobs...");

    ryot_log!(trace, "Invalidating invalid media import jobs");
    invalidate_import_jobs(&service.0).await.trace_ok();
    ...
    remove_old_entities_from_monitoring_collection(&service.0)
        .await
        .trace_ok();
    ryot_log!(trace, "Checking and queuing any pending reminders");
    service.queue_pending_reminders().await.trace_ok();
    ryot_log!(trace, "Recalculating calendar events");
    service.recalculate_calendar_events().await.trace_ok();
    ryot_log!(trace, "Queuing notifications for released media");
    service
        .queue_notifications_for_released_media()
        .await
        .trace_ok();
    ...
    service.regenerate_user_summaries().await.trace_ok();
    ...
    queue_notifications_for_outdated_seen_entries(&service.0)
        .await
        .trace_ok();
    ryot_log!(trace, "Removing useless data");
    remove_useless_data(&service.0).await.trace_ok();
    ryot_log!(trace, "Putting entities in partial state");
    put_entities_in_partial_state(&service.0).await.trace_ok();
    ...
    service.revoke_invalid_access_tokens().await.trace_ok();
    ryot_log!(trace, "Expiring cache keys");
    expire_cache_keys(&service.0).await.trace_ok();
