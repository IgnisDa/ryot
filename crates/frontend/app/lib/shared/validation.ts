import type { MediaCollectionFilter } from "@ryot-app/generated/graphql/backend/graphql";
...
export const zodDateTimeString = z.string().transform((v) => convertTimestampToUtcString(v));
...
export const parseAsCollectionsFilter = createParser<MediaCollectionFilter[]>({
	eq: (a, b) => isEqual(a, b),
	serialize: (value) => value.map((v) => `${v.collectionId}|${v.presence}|${v.strategy}`).join(","),
	parse: (value) =>
		value.split(",").map((v) => {
