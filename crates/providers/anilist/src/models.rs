...
mod tests {
    ...
    use super::{GraphQLResponse, MediaSearchResponse, parse_media_search_response};

    #[test]
    fn parse_media_search_response_handles_null_data() {
        let response: GraphQLResponse<MediaSearchResponse> =
            serde_json::from_value(json!({ "data": null })).unwrap();

        let (items, total) = parse_media_search_response(response);
    ...
    #[test]
    fn parse_media_search_response_keeps_valid_items() {
        let response: GraphQLResponse<MediaSearchResponse> = serde_json::from_value(json!({
            "data": {
                "Page": {
                    "pageInfo": { "total": 1 },
                    "media": [{
                        ...
                        "coverImage": { "extraLarge": "https://image.test/cover.jpg" },
                        "startDate": { "year": 2013 }
                    }]
                }
            }
        ...
        assert_eq!(items[0].publish_year, Some(2013));
    }
}
