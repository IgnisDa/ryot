...
import { getAuthErrorDiagnostic, getAuthErrorMessage, type AuthOperation } from "./errors";
...
describe("getAuthErrorDiagnostic", () => {
	it("returns only allowlisted diagnostic metadata", () => {
		...
		const diagnostic = getAuthErrorDiagnostic("reset-password", error);
		...
		expect(JSON.stringify(diagnostic)).not.toMatch(/reset-secret|hunter2|123456|session-secret/);
	});
	...
	])("excludes JSON-formatted and nested secrets", (cause) => {
		const diagnostic = getAuthErrorDiagnostic("reset-password", cause);
