...
export const MediaMonitoringGroup = HttpApiGroup.make("mediaMonitoring")
	.annotate(OpenApi.Description, "Manage media monitoring for entities.")
	.addError(Unauthorized, { status: 401 })
	.addError(RateLimited, { status: 429 })
	.middleware(AuthMiddleware)
	.add(
		HttpApiEndpoint.get("status")`/media-monitoring/${entityIdParam}`
			.annotate(OpenApi.Description, "Get the media monitoring status for an entity.")
			.addSuccess(MediaMonitoringStatus)
			.addError(NotFound, { status: 404 }),
	)
	.add(
		HttpApiEndpoint.put("enable")`/media-monitoring/${entityIdParam}`
			.annotate(OpenApi.Description, "Enable media monitoring for an entity.")
			.addSuccess(MediaMonitoringStatus)
			.addError(NotFound, { status: 404 }),
	)
	.add(
		HttpApiEndpoint.del("disable")`/media-monitoring/${entityIdParam}`
			.annotate(OpenApi.Description, "Disable media monitoring for an entity.")
			.addSuccess(MediaMonitoringStatus)
			.addError(NotFound, { status: 404 }),
	);
