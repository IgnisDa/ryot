...
export type DailyUserActivitiesQuery = { dailyUserActivities: { totalCount: number, mostActiveHour: number, items: Array<{ date: string, totalCounts: number, reviewCounts: number, workoutCounts: number, measurementCounts: number, hourCounts: Array<{ hour: number, count: number }>, metadataCounts: Array<{ lot: MediaLot, count: number }> }> } };
