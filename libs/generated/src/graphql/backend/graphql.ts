...
export type DailyUserActivitiesQuery = { dailyUserActivities: { groupedBy: DailyUserActivitiesResponseGroupedBy, totalCount: number, totalDuration: number, items: Array<{ day: string, bookCount: number, showCount: number, movieCount: number, animeCount: number, mangaCount: number, workoutCount: number, podcastCount: number, audioBookCount: number, videoGameCount: number, totalReviewCount: number, visualNovelCount: number, userMeasurementCount: number }> } };
