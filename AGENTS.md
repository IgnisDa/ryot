...
- Add dependencies from the target app with `bun add`; Bun pins exact versions.
...
- Do not add unrequested functionality, abstractions, or generalization.
- Derive types from schemas and existing types instead of writing mirrors. Frontend packages use Zod; `app-backend` and `@ryot/contract` use Effect Schema.
- Build application-owned query documents with `@ryot/query-engine` and use named recipes when available.
- Prefer `match` from `ts-pattern` over `switch`.
- Before writing code, use an `explore` subagent only to find existing patterns. It must not make decisions or create files.
- Avoid comments unless necessary.
- Omit return types when inference is sufficient.
- Order fields and variables by ascending line length unless semantic grouping is clearer. This does not apply to imports or parameters.
...
- Group related changes into logical commits; messages explain why.
