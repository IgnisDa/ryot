	...
	"SandboxProcessManager",
	{
		make: Effect.gen(function* () {
			...
			const complete = (worker: SandboxProcess, outcome: SandboxProcessOutcome) =>
				Effect.sync(() => activePids.delete(Number(worker.process.pid))).pipe(
			...
			const readWorkerMemory = () => {
				const pids = Array.from(activePids);
				if (process.platform === "linux") {
					return Effect.forEach(pids, readProcessRssBytes, { concurrency: "unbounded" }).pipe(
						Effect.map((values) => values.reduce((total: number, rss) => total + (rss ?? 0), 0)),
					);
				}
				return Effect.sync(() => {
					try {
			...
			yield* Effect.gen(function* () {
				...
				const workerRssBytes = yield* readWorkerMemory();
				const backend = process.memoryUsage();
				yield* recordSandboxRuntimeGauges({
					workerRssBytes,
					backendRssBytes: backend.rss,
					heapUsedBytes: backend.heapUsed,
					externalMemoryBytes: backend.external,
					activeExecutions: activeSandboxExecutions,
				});
