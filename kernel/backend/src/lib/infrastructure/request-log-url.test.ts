...
it.effect("uses route templates for every request method on protected paths", () =>
	Effect.gen(function* () {
		const url = "http://server.test/api/plugin-artifact-sessions/private-token/index.html";
		expect(yield* resolve(url)).toBe("/api/plugin-artifact-sessions/:token/:fileName");
		expect(yield* resolve(url, "OPTIONS")).toBe("/api/plugin-artifact-sessions/:token/:fileName");
	}),
);
