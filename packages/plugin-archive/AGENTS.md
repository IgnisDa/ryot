...
- Archives contain `manifest.json` first, then `backend/**` and canonically supported `client/**` files sorted by JavaScript code units. Import the client file policy from `@ryot/contract/modules/plugins/client`; do not duplicate it here.
