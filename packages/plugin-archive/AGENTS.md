...
- Archives contain `manifest.json` first, then `backend/**` and `client/**` source paths sorted with `localeCompare`; `client/**` entries must use `.ts`, `.tsx`, `.css`, or `.svg` extensions.
