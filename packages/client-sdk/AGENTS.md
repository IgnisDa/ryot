...
- `uploads` is a capability, not a protocol: one call hides intent creation, byte transfer, and completion, so no intent id, upload URL, or completion step reaches a caller. The `MessageChannel` adapter does not provide it, because every bridge payload is a `JsonValue` and a `Blob` cannot cross the port; a plugin calling it gets `unsupported-capability`.
