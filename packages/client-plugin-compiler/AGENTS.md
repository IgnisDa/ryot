...
- Keep the trusted bare imports exact: `clsx`, `react`, `react-dom`, `react-dom/client`, `react/jsx-runtime`; `@ryot-app/client-sdk` plus `/effect`, `/plugin`, `/react`, `/ryotql`, and `/screen`; `@ryot-app/client-ui-sdk` plus `/icon`, `/schema-form`, `/sync`, `/table`, and `/tint`; and `@ryot-app/plugin-kit/{effect,ryotql,schema}`. Reject every other bare import.
