...
- The trusted module allowlist (`react`, `react-dom`, `react-dom/client`, `react/jsx-runtime`, the published `@ryot/client-sdk` entry points, and `@ryot/client-ui-sdk`) is fixed; any other bare import must fail compilation, never silently pass through.
