...
- The trusted module allowlist (`clsx`, `react`, `react-dom`, `react-dom/client`, `react/jsx-runtime`, the published `@ryot-app/client-sdk` entry points, and `@ryot-app/client-ui-sdk`) is fixed; any other bare import must fail compilation, never silently pass through.
