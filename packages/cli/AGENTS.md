...
- Keep archives limited to `manifest.json`, non-test TypeScript sources under `backend/`, and non-test files under `client/` allowed by the canonical contract client file policy. A plugin's `host/` tree is never archived.
