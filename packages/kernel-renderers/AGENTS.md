...
- Emit renderer sources through `bun run generate`; never hand-edit `sources.generated.ts` and never read renderer files at runtime, because `apps/server` bundles.
