...
describe("notification channel CRUD", () => {
	it("creates a channel and returns a safe description", async () => {
		const { client } = await createAuthenticatedClient();
		await createNotificationChannel(client, {
			kind: "apprise",
			specifics: { baseUrl: fakeApprise.url, key: "secret-key", kind: "apprise" },
		});
		...
		expect(channel.kind).toBe("apprise");
	...
	it("updates disabled state", async () => {
		...
		const { id } = await createNotificationChannel(client, {
			kind: "telegram",
			specifics: { botToken: "bot-secret", chatId: "1234", kind: "telegram" },
		});
	...
	it("lists only the authenticated user's channels and protects mutations", async () => {
		...
		const { id } = await createNotificationChannel(owner.client, {
			kind: "push_safer",
			specifics: { key: "secret-key", kind: "push_safer" },
		});
	...
	it("rejects kind and specifics mismatches", async () => {
		const { client } = await createAuthenticatedClient();
		const mismatch = await client.runError((c) =>
			c.notifications.createChannel({
				payload: {
					kind: "email",
					specifics: { baseUrl: fakeApprise.url, key: "key", kind: "apprise" },
				},
			}),
		);
...
describe("notification delivery", () => {
	it("delivers to enabled channels in the background and skips disabled ones", async () => {
		...
		await createNotificationChannel(client, {
			kind: "apprise",
			specifics: { baseUrl: fakeApprise.url, key: "success", kind: "apprise" },
		});
		await createNotificationChannel(client, {
			kind: "apprise",
			specifics: { baseUrl: fakeApprise.url, key: "fail", kind: "apprise" },
		});
		await createNotificationChannel(client, {
			isDisabled: true,
			kind: "apprise",
