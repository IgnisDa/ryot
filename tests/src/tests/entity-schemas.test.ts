...
import { EntitySchemaId, TrackerId } from "@ryot/app-backend/schema/brands";
...
describe("GET /entity-schemas", () => {
	...
	it("returns empty array for custom tracker with no schemas", async () => {
		...
		const { trackerId } = await createTracker(client, { name: "Empty Tracker" });
	...
	it("returns 404 when attempting to access another user's custom tracker", async () => {
		...
		const { trackerId } = await createTracker(client1, { name: "User 1 Tracker" });
	...
	it("returns 200 when filtering by a single slug", async () => {
		...
		const { trackerId } = await createTracker(client, { name: "Single Slug Tracker" });
		await createEntitySchema(client, {
			trackerId,
			name: "Only Schema",
			slug: "only-schema",
	...
	it("lists schemas by slug across accessible trackers", async () => {
		...
		const { trackerId: booksTrackerId } = await createTracker(client, { name: "Books" });
	...
	it("returns all accessible schemas when trackerId and slugs are both missing", async () => {
		...
		const { trackerId } = await createTracker(client, { name: "Unfiltered Tracker" });
		await createEntitySchema(client, {
			trackerId,
			name: "Custom Entry",
			slug: "custom-entry",
	...
	it("custom schemas without linked scripts have providers as empty array", async () => {
		const { client } = await createAuthenticatedClient();
		const { trackerId } = await createTracker(client, { name: "Provider Test Tracker" });
		await createEntitySchema(client, {
			trackerId,
