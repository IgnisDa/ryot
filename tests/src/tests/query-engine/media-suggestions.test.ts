...
describe("Query engine media suggestions", () => {
	...
	it("builds collection recommendations from persisted media-suggestion edges without excluding existing members", async () => {
		const { client } = await createAuthenticatedClient();
		const collection = await createCollection(client, {
			name: `Suggestion Collection ${crypto.randomUUID()}`,
		});
		const { schema } = await createGlobalBookEntityFixture(client);
		const [sourceA, sourceB, candidateTop, candidateExisting] = await Promise.all([
			createGlobalBookEntityFixture(client, {
