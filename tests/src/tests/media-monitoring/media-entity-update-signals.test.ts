...
function pollNotificationBodies(key: string, count: number) {
	return pollUntil(`${count} notification(s) for '${key}'`, () => {
		const requests = fakeApprise.requests.filter(({ path }) => path === `/notify/${key}`);
		return Promise.resolve(requests.length === count ? requests : null);
	});
}

describe("hierarchical media entity-update signals", () => {
	it("notifies content-count and publish-year changes for a monitored anime", async () => {
		...
		const animeProvider = await seedBuiltinProviderScript({
			client,
			slug: animeSlug,
		...
		try {
			...
			const monitor = await createAuthenticatedClient();
			await createNotificationChannel(monitor.client, {
				channel: "apprise",
				channelSpecifics: { baseUrl: fakeApprise.url, key: "anime-monitor", kind: "apprise" },
			});
			await enableMediaMonitoring(monitor.client, anime.id);

			fakeApprise.requests.length = 0;
			await triggerCronAndWaitForEntity(anime.id);
			...
			const delivered = await pollNotificationBodies("anime-monitor", 2);
	...
	it("notifies episode name, image, and release-date changes while staying silent for a special season", async () => {
		...
		const showProvider = await seedBuiltinProviderScript({
			client,
			slug: showSlug,
			drivers: {
		...
		try {
			...
			await enableMediaMonitoring(monitor.client, show.id);

			fakeApprise.requests.length = 0;
			await triggerCronAndWaitForEntity(show.id);
			expect(
			...
			const delivered = await pollNotificationBodies("show-episode-monitor", 3);
			const bodies = delivered
				.map(({ body }) => requireObjectRecord(body, "Missing notification body").body)
				.sort((left, right) => String(left).localeCompare(String(right)));
			expect(bodies).toEqual(
			...
			await cleanupBuiltinProviderScript(showProvider);
		}
	});
});
