...
The SQL allowlist is workflow polling through `cluster_messages`/`cluster_replies`, the deleted tracker assertion in `god-mode/delete-user.test.ts`, `queryUserEntityStateCounts`, `countMediaMonitoringRelationships`, and the signal/recipient/run/rule helpers in `fixtures/automations.ts` (`querySignalId`/`pollSignalId`, `querySignalRecipientUserIds`/`pollSignalRecipientCount`, `querySubscriptionRunStatuses`/`pollTerminalSubscriptionRunStatuses`, `queryAutomationRuleCount`) — signal, recipient, and run history have no listing endpoint (deferred per the automations PRD), so account-deletion cleanup and subscription-run completion can only be observed directly. Document any new exception here; otherwise use the contract API. `tests/src/seed-script.ts` remains excluded from fixture cleanup.
