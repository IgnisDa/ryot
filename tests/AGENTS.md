...
The SQL allowlist is workflow polling through `cluster_messages`/`cluster_replies`, the deleted tracker assertion in `god-mode/delete-user.test.ts`, `queryUserEntityStateCounts`, `countMediaMonitoringRelationships`, and the rule helper in `fixtures/automations.ts` (`queryAutomationRuleCount`) — rule history has no listing endpoint (deferred per the automations PRD), so its cleanup can only be observed directly. Signals and their recipients are inspected through the admin `testSupport.listSignals` endpoint (`listSignals`/`pollSignal`/`pollSignalWithRecipientCount` in `fixtures/automations.ts`), and subscription runs through `testSupport.listSubscriptionRuns` (`listSubscriptionRuns`/`pollTerminalSubscriptionRuns`), not SQL. Document any new exception here; otherwise use the contract API. `tests/src/seed-script.ts` remains excluded from fixture cleanup.
