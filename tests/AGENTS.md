...
The SQL allowlist is workflow polling through `cluster_messages`/`cluster_replies`, the deleted tracker assertion in `god-mode/delete-user.test.ts`, `queryUserEntityStateCounts`, `countMediaMonitoringRelationships`, and the run/rule helpers in `fixtures/automations.ts` (`querySubscriptionRunStatuses`/`pollTerminalSubscriptionRunStatuses`, `queryAutomationRuleCount`) — subscription run and rule history have no listing endpoint (deferred per the automations PRD), so their cleanup and completion can only be observed directly. Signals and their recipients are inspected through the admin `testSupport.listSignals` endpoint (`listSignals`/`pollSignal`/`pollSignalWithRecipientCount` in `fixtures/automations.ts`), not SQL. Document any new exception here; otherwise use the contract API. `tests/src/seed-script.ts` remains excluded from fixture cleanup.
