...
export function useTrackerOverviewData(input: {
	...
	const facetSavedViews = savedViewsQuery.savedViews.filter(
		(view) => view.facetId === input.facet.id,
	);
	...
	const schemaSummaries = input.entitySchemas.map((schema) => {
		...
		const savedView = facetSavedViews.find((view) =>
