...
const withOpenApiGeneratedBanner = (types: string) =>
	types.startsWith(openApiGeneratedBanner)
		? types
		: `${openApiGeneratedBanner}${types}`;
...
const openApiTypesPlugin = (): Plugin => {
	...
	const syncOpenApiTypes = async () => {
		...
		try {
			...
			const nextTypes = withOpenApiGeneratedBanner(astToString(ast));
