export const SECTION_CONTAINER = "max-w-7xl mx-auto px-4 sm:px-6 lg:px-8";
export const SECTION_CONTAINER_NARROW = "max-w-4xl mx-auto px-4 sm:px-6 lg:px-8";
export const CARD_HOVER = "hover:shadow-lg transition-all duration-300 hover:-translate-y-1";
export const FEATURE_CARD = `border-2 rounded-xl hover:border-primary/20 ${CARD_HOVER}`;
export const SECTION_Y_PADDING = "py-20";
export const SECTION_Y_PADDING_LARGE = "py-20 lg:py-32";
