...
export const RegistrationSection = (props: RegistrationSectionProps) => {
	...
	if (props.isLoading || !props.configData) {
		content = <LoadingSpinner message="Loading registration form..." />;
	} else if (props.configData.isLoggedIn) {
		content = (
			<Link to={$path("/me")}>
			...
			<>
				...
				>
					{props.query.email ? (
						...
						<>
							<input type="hidden" name="turnstileToken" value={props.loginOtpTurnstileToken} />
							<Input
								type="email"
								name="email"
	...
	return (
		<section id="start-here" className="py-20 bg-muted/30">
			<div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
				...
				/>
				<div className="mt-8" />
				<div className="max-w-sm mx-auto space-y-4">{content}</div>
			</div>
		</section>
	);
};
