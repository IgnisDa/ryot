...
describe("backups screen", () => {
	...
	it("holds only the row whose archive is being prepared", async () => {
	...
	it("leads with a live backup and refuses to start a second one", async () => {
		await renderView(readyState([liveRun]));
		...
		expect(screen.getByText("Started 1 hour ago")).toBeOnTheScreen();
		expect(
			screen.getByText("This keeps running on your server, even if you close Ryot."),
		).toBeOnTheScreen();
		expect(
			screen.getByText("A backup is already under way. You can start another when it finishes."),
		).toBeOnTheScreen();
		expect(screen.getByRole("button", { name: "Create a backup" })).toBeDisabled();
		expect(
			screen.queryByRole("button", { name: "Delete the backup record from 1 hour ago" }),
		).not.toBeOnTheScreen();
	});
