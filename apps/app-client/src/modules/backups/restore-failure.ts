...
/**
 * Only the cleanliness conflict is reported when the restore is created. The upload token is
 * claimed inside the dispatched workflow, so a stale or unreadable archive surfaces later as the
 * run's own error and is worded by `backupRunFailureNotice`.
 */
