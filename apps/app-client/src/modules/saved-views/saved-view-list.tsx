			...
			{props.items.map((item) => (
				<Link asChild key={item.entityId} href={getEntityHref(item.entityId)}>
					<Pressable
						...
						className="min-h-28 flex-row items-center gap-3 border-b border-border py-2 focus-visible:outline-2 focus-visible:outline-accent md:min-h-18 md:gap-3.5 md:px-1"
					>
						<SavedViewImageView
							image={item.image}
