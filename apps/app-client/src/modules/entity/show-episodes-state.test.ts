...
const readySeasons = (seasons: SeasonInput) => {
	const state = mapShowEpisodes(AsyncResult.success(decodeShowEpisodesResult({ seasons })));
...
describe("show episodes state", () => {
	it("maps a pending query to the loading state", () => {
		expect(mapShowEpisodes(AsyncResult.initial(true))).toEqual({ status: "loading" });
	});

	it("maps a malformed decode failure apart from a transport failure", () => {
		const malformed = Cause.fail(new RyotQLMalformedResultError("bad episode row"));
		const transport = Cause.fail(new Error("offline"));

		expect(mapShowEpisodes(AsyncResult.failure(malformed)).status).toBe("malformed");
	...
	it("selects loaded episode IDs for entity interest", () => {
		const loading = mapShowSeasonEpisodes(AsyncResult.initial(true));
		const ready = mapShowSeasonEpisodes(
			AsyncResult.success(
				decodeShowSeasonEpisodesResult({
					episodes: [showEpisodeRow, episode({ id: "episode-2", episodeNumber: 2 })],
				}),
			),
		);
	...
	it("reports a show without seasons or without a row as empty", () => {
		expect(mapShowEpisodes(AsyncResult.success(decodeShowEpisodesResult({ seasons: [] })))).toEqual(
			{ status: "empty" },
		);
		expect(mapShowEpisodes(AsyncResult.success(decodeShowEpisodesResult({ show: null })))).toEqual({
			status: "empty",
		});
	});
