...
- `table.imageField` is independent from the grid and list image fields. Render it as an unlabeled leading visual associated with the first textual column; it is not a data column. Render `table.columns` in their configured order.
- A null field mapping means that the layout has no such slot. A null row value omits non-image content. When a configured image field has a null or unusable row value, preserve the image region's dimensions and show the shared missing-image placeholder.
