...
const validateStrictPrimaryEventRefsInPredicate = (
	...
	context: QueryEngineReferenceContext,
	validBuiltins: ReadonlySet<string>,
	computedFields: ReadonlyArray<QueryComputedField> | undefined,
) => {
...
const validateEntityQueryEngineReferences = (
	...
	if (sort) {
		...
		assertSortableExpression(
			inferViewExpressionType({ context, computedFieldMap, expression: sort.expression }),
		);
	}
...
const validateAggregateQueryEngineReferences = (
	...
	for (const field of request.aggregations) {
		...
		if (aggregation.type === "countBy") {
			...
			assertComparableExpression(
				inferViewExpressionType({ context, computedFieldMap, expression: aggregation.groupBy }),
				"countBy",
			);
			continue;
		}
		...
		assertNumericExpression(
			inferViewExpressionType({ context, computedFieldMap, expression: aggregation.expression }),
...
const validateTimeSeriesQueryEngineReferences = (
	...
	if (request.metric.type === "sum") {
		...
		assertNumericExpression(
			inferViewExpressionType({ context, computedFieldMap, expression: request.metric.expression }),
			"sum metric",
		);
	}
};
...
export const validateSavedViewDisplayConfiguration = (
	...
	for (const ref of [
		...
		if (ref) {
			validateExpressionAgainstSchemas(ref, context, displayBuiltins, computedFieldMap);
		}
	}
