...
import type { AppPropertyDefinition, AppSchema } from "#lib/schema/core";
...
export const getEventJoinForReference = <TJoin extends QueryEngineEventJoinLike>(
	...
	if (!foundJoin) {
		throw new QueryEngineValidationError(
			`Event join 'event.${reference.joinKey}' is not part of this runtime request`,
		);
	}
...
export const getRelationshipJoinForReference = <TJoin extends QueryEngineRelationshipJoinLike>(
	...
	if (!foundJoin) {
		throw new QueryEngineValidationError(
			`Relationship join 'relationship.${reference.joinKey}' is not part of this runtime request`,
		);
	}
