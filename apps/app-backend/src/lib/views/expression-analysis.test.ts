...
describe("inferViewExpressionType", () => {
	it("infers arithmetic, concat, and numeric normalization result types", () => {
		...
		expect(
			...
			kind: "property",
			propertyType: "string",
			propertyDefinition: { label: "Value", type: "string", description: "Computed text value" },
		});
	...
	it("unifies conditional branches and keeps image expressions display-only", () => {
		const imageExpression = {
			...
			reference: { path: ["image"], slug: "smartphones", type: "entity" as const },
		};
	...
	it("infers computed-field references and rejects missing computed fields", () => {
		...
		expect(
			inferViewExpressionType({
				context,
				computedFieldMap,
				expression: createComputedFieldExpression("displayName"),
			}),
		).toEqual({
			kind: "property",
			propertyType: "string",
		...
		expect(() =>
			inferViewExpressionType({
				context,
				expression: createComputedFieldExpression("missingField"),
			}),
	...
	it("infers string type for event-schema.slug column", () => {
		expect(
			inferViewExpressionType({
				context,
				expression: { type: "reference", reference: { type: "event-schema", path: ["slug"] } },
			}),
		).toMatchObject({ kind: "property", propertyType: "string" });
	});
	...
	it("infers boolean type for event-schema.isBuiltin column", () => {
		expect(
			inferViewExpressionType({
				context,
				expression: { type: "reference", reference: { type: "event-schema", path: ["isBuiltin"] } },
			}),
	...
	it("rejects unsupported event built-in columns", () => {
		expect(() =>
			inferViewExpressionType({
				context,
				expression: { type: "reference", reference: { type: "event", path: ["unknownColumn"] } },
			}),
