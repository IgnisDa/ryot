...
- **Plugin and kernel scripts** are TypeScript `.sandbox.ts` ES modules. Plugin ingestion compiles package source, persists immutable content-addressed rows, and loads the active hashes. `sandbox:compile` only generates ignored runner-integration artifacts under `generated-sandbox/`; boot does not seed or execute that registry. There is no legacy `.sandbox.js` fragment, `code` column, or format-0 path.
