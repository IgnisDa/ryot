...
| User                    | `changeUserRelationships`, `createEvents`, `ensureUserEntities`, `executeQueryEngine`, `getEntity`, `getEntitySchema`, `getIntegration`, `getUserPreferences`, `listEventSchemas`, `listEvents`, `listIntegrations` |
...
wins between materialization and linking. Scope cleanup removes execution link after process exits.
