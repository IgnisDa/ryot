...
- Sandbox scripts and script-side helpers (`automations/`, `providers/`, `script-helpers/`) live under `modules/builtins/sandbox-scripts/`, not here — this folder is only the execution runtime.
...
- **Built-ins** are TypeScript `.sandbox.ts` ES modules too (52 providers + 14 automations under `src/modules/builtins/sandbox-scripts/`, sharing helpers via ordinary relative imports from `sandbox-scripts/script-helpers/`). `sandbox:compile` compiles each into the ignored `src/modules/builtins/generated-sandbox/` directory as a format-1 `.mjs` plus a `registry.ts` carrying the source, compiled code, and validated manifest. Seeding reads that registry; there is no legacy `.sandbox.js` fragment, `code` column, or format-0 path.
