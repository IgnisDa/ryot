...
import { Clock, Deferred, Effect, Fiber, Layer, Queue, Ref, Semaphore } from "effect";
...
const makeBridgeLayer = () => {
	...
	return BridgeService.layer.pipe(Layer.provide(Layer.succeed(RedisService, redis)));
};
...
const requestBridge = (bridge: BridgeService["Service"], executionId: string) =>
