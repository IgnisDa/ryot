...
driver("details", async function (context, { metadata }) {
	...
	const slug = typeof movie?.slug === "string" && movie.slug.trim() ? movie.slug.trim() : null;
	const sourceUrl = slug ? `https://thetvdb.com/movies/${slug}` : null;

	const groupRelatedEntities = Array.isArray(movie?.lists)
		? movie.lists
