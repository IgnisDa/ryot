...
async function getMetronCredentials() {
	...
	const passwordResponse = await getAppConfigValue("providers.metronPassword");
...
function buildPeople(credits) {
	...
	for (const credit of credits) {
		...
		const externalId = getIdentifier(credit.id);
		const name = getString(credit.creator) ?? "Loading...";
		if (!externalId) {
			continue;
		}
		...
		const roleNames = Array.isArray(credit.role)
			? credit.role
					.map((role) => (role && typeof role === "object" ? getString(role.name) : null))
...
driver("details", async function (context) {
	...
	const payload = await loadMetronJson(
		`https://metron.cloud/api/issue/${encodeURIComponent(externalId)}/`,
			...
			? [
					{
						...
						externalId: getIdentifier(payload.series.id),
						name: getString(payload.series.name) ?? "Loading...",
						relationshipProperties: {
							roles: ["Member"],
						},
					},
