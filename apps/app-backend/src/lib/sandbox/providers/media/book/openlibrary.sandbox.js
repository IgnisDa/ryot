		...
		.map((doc) => {
			...
			const publishYear =
				typeof doc?.first_publish_year === "number" && Number.isFinite(doc.first_publish_year)
					...
					: null;
			const coverId =
				typeof doc?.cover_i === "number" && Number.isFinite(doc.cover_i) ? doc.cover_i : null;

			const imageUrl =
				coverId === null
					? null
					: "https://covers.openlibrary.org/b/id/" + String(coverId) + "-M.jpg?default=false";
...
driver("details", async function (context) {
	...
	const editions = Array.isArray(editionsPayload?.entries) ? editionsPayload.entries : [];
	...
	for (const entry of editions) {
		if (typeof entry?.number_of_pages === "number" && Number.isFinite(entry.number_of_pages)) {
...
driver("resolve", async function (context) {
	...
	if (!value) {
		throw new Error("OpenLibrary resolve requires a non-empty isbn value");
	}

	const response = await httpCall("GET", `https://openlibrary.org/isbn/${value}.json`);
	if (!response?.success) {
	...
	const workKey = Array.isArray(payload?.works)
		? payload.works.map((work) => work?.key).find(Boolean)
		: undefined;
	const fromWorks = getKeySegment(workKey);
	const fromKey =
		typeof payload?.key === "string" && payload.key.startsWith("/works/")
			? getKeySegment(payload.key)
			: "";
