...
function collectUnlinkedCreators(authors, publisher) {
	...
	if (Array.isArray(authors)) {
		for (const author of authors) {
			...
			unlinkedCreators.push({ role: "Author", name: trimmedAuthor });
		}
	}
	...
	if (typeof publisher === "string") {
		...
		if (trimmedPublisher) {
			...
			if (!seen.has(key)) {
				seen.add(key);
				unlinkedCreators.push({ role: "Publisher", name: trimmedPublisher });
			}
		}
	}
...
async function getGoogleBooksApiKey() {
	const configValueResponse = await getAppConfigValue("providers.googleBooksApiKey");
	if (!configValueResponse?.success) {
		throw new Error(configValueResponse?.error ?? "Could not load Google Books API key");
	}
...
driver("search", async function (context) {
	...
	const items = volumes
		.map((volume) => {
			const externalId = typeof volume?.id === "string" ? volume.id : "";
			if (!externalId) {
				return null;
			}

			const volumeInfo =
				volume?.volumeInfo && typeof volume.volumeInfo === "object" ? volume.volumeInfo : null;
...
driver("details", async function (context) {
	...
	const externalId =
		typeof payload?.id === "string" && payload.id.trim() ? payload.id : contextIdentifier;

	const volumeInfo =
		payload?.volumeInfo && typeof payload.volumeInfo === "object" ? payload.volumeInfo : null;
	const title = typeof volumeInfo?.title === "string" ? volumeInfo.title : "";
	if (!title) {
	...
	return {
		...
		properties: {
			...
			images: collectImages(volumeInfo?.imageLinks).map((url) => ({ type: "remote", url })),
			...
			unlinkedCreators: collectUnlinkedCreators(volumeInfo?.authors, volumeInfo?.publisher),
			description: typeof volumeInfo?.description === "string" ? volumeInfo.description : null,
		},
	};
});
...
driver("resolve", async function (context) {
	...
	if (!value) {
		throw new Error("Google Books resolve requires a non-empty isbn value");
	}
	...
	if (!response?.success) {
		throw new Error(response?.error ?? "Google Books ISBN lookup failed");
	}
