...
function getTrackTitle(trackItem) {
	const title = trackItem?.title?.text ?? trackItem?.title ?? "";
...
driver("search", async function (context) {
	...
	for (const shelf of results.contents ?? []) {
		for (const track of shelf.contents ?? []) {
			...
			const thumb = getBestThumbnailUrl(track.thumbnail);

			allItems.push({
				externalId: String(id),
				calloutProperty: { kind: "null", value: null },
				titleProperty: { kind: "text", value: String(title) },
				primarySubtitleProperty:
					year === null ? { kind: "null", value: null } : { kind: "number", value: Number(year) },
...
driver("details", async function (context, { metadata }) {
	...
	const queue = await yt.music.getUpNext(externalId);

	const contents = queue.contents ?? [];
	const trackItem = contents.find((item) => item.video_id === externalId) ?? contents[0];
	...
	const publishYear = trackItem.album?.year
		? Number.parseInt(String(trackItem.album.year), 10) || null
		: null;
	...
	for (const artist of trackItem.artists ?? []) {
		const artistId =
			typeof artist?.channel_id === "string" && artist.channel_id.trim()
		...
		const artistName =
			typeof artist?.name === "string" && artist.name.trim() ? artist.name.trim() : "Loading...";
	...
	const byVariousArtists = relatedEntityByKey.size === 0 ? null : relatedEntityByKey.size > 1;

	const album = trackItem.album;
...
driver("translate", async function (context) {
	...
	const queue = await yt.music.getUpNext(externalId);
	const contents = queue.contents ?? [];
	const trackItem = contents.find((item) => item.video_id === externalId);
	if (!trackItem) {
		throw new Error(`YouTube Music track not found: ${externalId}`);
	}

	const name = getTrackTitle(trackItem);
	return name ? { name } : {};
});

driver("history", async function (context) {
	const { z } = await import("npm:zod");
	const { authCookie, timezone } = z
		.object({
	...
	const { Innertube, YTNodes } = await import("npm:youtubei.js");
	const yt = await Innertube.create({ cookie: authCookie, fetch: makeFetch });
