...
async function getCredentials() {
	const [clientIdResp, clientSecretResp] = await Promise.all([
		getAppConfigValue("providers.twitchClientId"),
		getAppConfigValue("providers.twitchClientSecret"),
	]);
	...
	const clientId = typeof clientIdResp.data === "string" ? clientIdResp.data.trim() : null;
	const clientSecret =
		typeof clientSecretResp.data === "string" ? clientSecretResp.data.trim() : null;
...
function buildPlatformReleases(releaseDates, dayjs) {
	...
	const releases = releaseDates
		.map((rd) => {
			...
			const platformName =
				rd.platform && typeof rd.platform === "object" && typeof rd.platform.name === "string"
...
function collectCompanies(involvedCompanies) {
	...
	for (const ic of involvedCompanies) {
		if (!ic || typeof ic !== "object" || !ic.company) {
			continue;
		}

		const id =
			typeof ic.company.id === "number" && Number.isFinite(ic.company.id)
				? Math.trunc(ic.company.id)
				: null;
		if (id === null) {
			continue;
		}
		...
		const name =
			typeof ic.company.name === "string" && ic.company.name.trim()
...
function collectGroups(collections) {
	...
	for (const collection of collections) {
		const externalId =
			typeof collection?.id === "number" ? String(Math.trunc(collection.id)) : null;
		if (!externalId) {
			continue;
		}
...
driver("search", async function (context) {
	...
	const totalItems =
		typeof headers["x-count"] === "string" && Number.isFinite(Number(headers["x-count"]))
	...
	const items = results
		.map((game) => {
			...
			const externalId = typeof game.id === "number" ? String(game.id) : null;
			if (!externalId) {
				return null;
			}
...
driver("details", async function (context) {
	...
	const gameBody = [`fields ${DETAIL_FIELDS};`, `where id = ${externalId};`].join("\n");

	const ttbBody = ["fields normally, hastily, completely;", `where game_id = ${externalId};`].join(
		"\n",
	);
	...
	if (game.cover && typeof game.cover.image_id === "string") {
		images.push({ type: "remote", url: getImageUrl(game.cover.image_id) });
	}
	if (Array.isArray(game.artworks)) {
		for (const artwork of game.artworks) {
			if (artwork && typeof artwork.image_id === "string") {
				images.push({ type: "remote", url: getImageUrl(artwork.image_id) });
			}
		}
	}
	...
	const genres = Array.isArray(game.genres)
		? game.genres
				.map((g) => (g && typeof g.name === "string" ? g.name.trim() : null))
	...
	const ttbEntry = Array.isArray(ttbList) && ttbList.length > 0 ? ttbList[0] : null;
	...
	const providerRating =
		typeof game.rating === "number" && Number.isFinite(game.rating) ? game.rating : null;

	const description =
		typeof game.summary === "string" && game.summary.trim() ? game.summary.trim() : null;
