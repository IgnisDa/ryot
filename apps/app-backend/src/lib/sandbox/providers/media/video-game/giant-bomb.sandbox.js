...
async function getApiKey() {
	const configValueResponse = await getAppConfigValue("providers.giantBombApiKey");
	if (!configValueResponse?.success) {
		throw new Error(configValueResponse?.error ?? "Failed to retrieve GiantBomb API key");
	}

	const apiKey =
		typeof configValueResponse.data === "string" ? configValueResponse.data.trim() : null;
...
function collectNames(items) {
	...
	return items
		.map((item) =>
			item && typeof item === "object" && typeof item.name === "string" ? item.name.trim() : null,
		)
		.filter((name) => name !== null && name.length > 0);
}

function collectCompanies(developers, publishers) {
	const companyByKey = new Map();

	const addCompany = (items, role) => {
		if (!Array.isArray(items)) {
			return;
		}
		for (const item of items) {
			...
			const name =
				typeof item.name === "string" && item.name.trim() ? item.name.trim() : "Loading...";

			const key = `company.giant-bomb:${guid}`;
	...
	addCompany(developers, "Developer");
	addCompany(publishers, "Publisher");

	return [...companyByKey.values()];
}

function collectGroups(franchises) {
	...
	for (const franchise of franchises) {
		const externalId = extractGiantBombGuid(franchise?.api_detail_url);
		if (!externalId) {
			continue;
		}
		...
		const name =
			typeof franchise?.name === "string" && franchise.name.trim()
				? franchise.name.trim()
				: "Loading...";
		const key = `video-game-group.giant-bomb:${externalId}`;
...
function buildPlatformReleases(platforms) {
	...
	const releases = platforms
		.map((platform) => {
			...
			const name = typeof platform.name === "string" ? platform.name.trim() : null;
			if (!name) {
				return null;
			}
...
driver("search", async function (context) {
	...
	const items = results
		.map((game) => {
			...
			const externalId =
				typeof game.guid === "string" && game.guid.trim() ? game.guid.trim() : null;
			if (!externalId) {
				return null;
			}
...
driver("details", async function (context) {
	const contextIdentifier =
		typeof context?.externalId === "string" ? context.externalId.trim() : "";
	...
	const game = payload?.results && typeof payload.results === "object" ? payload.results : null;
	...
	const genres = [...collectNames(game.genres), ...collectNames(game.themes)];
