...
function parseAiringSchedule(airingSchedule, nextAiringEpisode, dayjs) {
	...
	if (nextAiringEpisode && typeof nextAiringEpisode === "object") {
		const episode =
			typeof nextAiringEpisode.episode === "number" && Number.isFinite(nextAiringEpisode.episode)
...
`;
	...
	const studioNodes =
		media.studios?.nodes && Array.isArray(media.studios.nodes) ? media.studios.nodes : [];
	const relatedEntities = [];
	const seenStudios = new Set();
	for (const studio of studioNodes) {
		...
		if (studioId === null || seenStudios.has(studioId)) {
			continue;
		}
		seenStudios.add(studioId);

		const studioName =
			typeof studio.name === "string" && studio.name.trim() ? studio.name.trim() : "Loading...";
...
`;
	...
	if (!response?.success) {
		throw new Error(response?.error ?? "Anilist anime translation request failed");
	}
	...
	const graphQlErrorMessage = extractGraphQlErrorMessage(payload);
	if (graphQlErrorMessage) {
		throw new Error(`Anilist anime translation GraphQL error: ${graphQlErrorMessage}`);
	}
