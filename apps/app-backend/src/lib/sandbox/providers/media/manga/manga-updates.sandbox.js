...
async function extractStatus(input) {
...
function collectGenres(genres, categories) {
	...
	if (Array.isArray(genres)) {
		for (const genreEntry of genres) {
			const genre = typeof genreEntry?.genre === "string" ? genreEntry.genre.trim() : "";
			if (genre) {
				genreSet.add(genre);
			}
		}
...
driver("search", async function (context) {
	...
	const response = await httpCall("POST", "https://api.mangaupdates.com/v1/series/search", {
...
driver("details", async function (context) {
	...
	const chapters =
		typeof payload?.latest_chapter === "number" && Number.isFinite(payload.latest_chapter)
	...
	return {
		...
		properties: {
			...
			providerRating:
				typeof payload?.bayesian_rating === "number" && Number.isFinite(payload.bayesian_rating)
