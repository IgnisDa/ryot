...
driver("details", async function (context, { metadata }) {
	...
	const [data, translationData] = await Promise.all([
		tvdbGet(`/people/${externalId}/extended`),
		tvdbGetOptional(`/people/${externalId}/translations/${language}`),
	]);
...
driver("translate", async function (context) {
	...
	const translationData = await tvdbGetOptional(`/people/${externalId}/translations/${language}`);
