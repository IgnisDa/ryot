...
driver("search", async function (context) {
	...
	const items = results
		.map((franchise) => {
			...
			const externalId =
				typeof franchise.guid === "string" && franchise.guid.trim() ? franchise.guid.trim() : null;
			if (!externalId) {
				return null;
			}

			const name =
				typeof franchise.name === "string" && franchise.name.trim() ? franchise.name.trim() : null;
			if (!name) {
				return null;
			}
...
driver("details", async function (context) {
	...
	const franchise =
		payload?.results && typeof payload.results === "object" ? payload.results : null;
	if (!franchise) {
