...
function getAlbumTitle(album) {
	const title = album?.title ?? album?.header?.title?.text ?? "";
...
driver("details", async function (context, { metadata }) {
	...
	const coverUrl = getLargestThumbnailUrl(album?.thumbnail ?? album?.header?.thumbnail);
	const images = coverUrl ? [{ type: "remote", url: coverUrl }] : [];

	const playlistId = typeof album?.playlist_id === "string" ? album.playlist_id : null;
...
driver("translate", async function (context) {
	...
	const yt = await createYoutubeMusicClient(language);
	const name = getAlbumTitle(await yt.music.getAlbum(externalId));
	return name ? { name } : {};
});
