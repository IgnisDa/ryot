...
driver("details", async function (context) {
	...
	const seriesResp = await httpCall("GET", `${baseUrl}/${externalId}?${seriesParams.toString()}`);
	...
	const sortedAsins = rawRelationships
		...
		.sort((a, b) => {
			const sa = typeof a.sort === "string" ? Number.parseInt(a.sort, 10) : 0;
			const sb = typeof b.sort === "string" ? Number.parseInt(b.sort, 10) : 0;
			return sa - sb;
		})
		.map((r) => r.asin);
