...
function normalizeOptionalEnumValue(value, validValues) {
	...
	return validValues.has(normalized) ? { ok: true, value: normalized } : { ok: false, value: null };
}
...
function getInstructions(value) {
	...
	return value.every((instruction) => typeof instruction === "string") ? value : null;
}
...
async function readCachedExercises() {
	...
	for (let index = 0; index < metadata.chunkCount; index += 1) {
		const chunk = await readCachedValue(`${CACHE_KEY}:${metadata.version}:chunk:${index}`);
...
async function writeCachedExercises(rows) {
	...
	for (let index = 0; index < chunks.length; index += 1) {
		await writeCachedValue(`${CACHE_KEY}:${version}:chunk:${index}`, chunks[index]);
	}
