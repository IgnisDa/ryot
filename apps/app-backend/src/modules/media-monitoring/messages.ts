...
const change = (message: string, discriminator = message): MediaMonitoringChange => ({
	message,
	fingerprint: fingerprint(discriminator),
});
...
export const mediaMonitoringMessages = {
	associationAdded: (input: {
		...
		const message = `${input.entityName} has been associated with ${input.associationName} as ${input.role}`;
		return change(message, `${input.entityKind}:${input.associationKind}:${message}`);
	},
	...
	statusChanged: (input: { entityName: string; newStatus: string; oldStatus: string }) =>
		change(`Status of ${input.entityName} changed from ${input.oldStatus} to ${input.newStatus}`),
};
