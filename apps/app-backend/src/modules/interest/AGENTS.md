...
- Wire schemas stay in `messages.ts`; the `reason` vocabulary stays in `#lib/redis`. Don't redefine either inline.
...
- Any new completion `reason` must be added to `EntityUpdatedReason` in `#lib/redis` and threaded through `handleRow`'s terminal mapping and both publisher workflows.
