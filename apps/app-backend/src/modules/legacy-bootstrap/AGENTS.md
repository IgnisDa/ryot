...
- SQL value lists inlined into DO blocks must go through `quoteSqlString` from `shared.ts` and must only contain controlled values (hardcoded mappings and IDs already read from our own database) — never user input.

## Slim Migration Strategy

The migration copies **user-generated data in full** and **provider-sourced ("global") data only as far as user data references it**. Provider entities (`metadata`, `metadata_group`, `person`/company and their season/episode children) are fully reconstructable on demand: V2's entity population workflow refetches an entity from its provider using the `(external_id, entity_schema_slug, provider_id)` tuple and rebuilds its `name`/`properties`, child entities, and all related-entity relationships (cast/crew, group membership, genres, media suggestions). A skeleton row (`populated_at = NULL`, `properties = {}`) is a first-class state that populates lazily through declared interest or a plugin workflow.

Provenance is the **logical provider** (`sandbox_provider.id`), not the script that executes it: `sandbox_script.id` is an execution identity only. Global provider-backed entities are unique — and therefore reconstructable — on `(external_id, entity_schema_slug, provider_id)` (`entity_global_external_id_unique`); user-scoped provider-backed entities additionally key on `user_id`, i.e. `(user_id, external_id, entity_schema_slug, provider_id)` (`entity_user_schema_provider_external_id_unique`). Both indexes live in `src/lib/infrastructure/db/schema/tables/entities.ts`.

Migration targets therefore resolve to provider ids: `migrate-data.ts` reads `sandbox_provider` and keeps only rows the **active plugin-loader snapshot** still declares (a persisted row whose plugin no longer declares that provider slug is stale, because repository upserts never delete declarations), mirroring `findActiveProvider` in `src/modules/plugins/runtime-resolver.ts`. An unresolvable target provider slug fails loudly.

Consequences that drive the design below:

- **Custom (user-authored) entities have no provider** (`provider_id IS NULL`), so they are neither reconstructable nor identified by the provider tuple. Their legacy ids are preserved verbatim and they are always migrated in full with their `properties`; the partial `entity_global_no_provider_external_id_unique` index covers global no-provider rows.
...
- After `old_user` is migrated, each legacy user is passed through `bootstrapNewUser` so migrated accounts receive plugin workspace state, saved views, a library entity, and the active notification subscriptions that auth-created users get.
- **Referenced provider entities are migrated as skeletons.** `metadata`, `metadata_group`, and `person`/company entity migrations filter provider rows (`provider_id IS NOT NULL`) to `_referenced_global_entity_ids` and emit `properties = '{}'::jsonb` with `populated_at = NULL`; V2 repopulates them from their provider on first view. `entity_schema_slug`/`provider_id` are derived from the V1 `lot`/`source`, and `external_id`/`name` are copied so the row is identifiable and reconstructable.
- **Custom entities are migrated in full.** Rows whose resolved `provider_id IS NULL` (V1 `source = 'custom'`) keep their full computed `properties` regardless of whether they are referenced, because there is no provider to rebuild them from.
- V1 show/podcast season and episode blobs are exploded into V2 `show-season`, `show-episode`, and `podcast-episode` entities by `episodic-sub-entity-mapping.ts`. Because the explosion joins the already-migrated parent `entity` row, it naturally covers only referenced/custom shows and podcasts. It runs immediately after metadata entities are migrated and before review/seen events, and writes the parent-child relationships plus the session-local episode resolution temp tables reused by review/seen migration. These children still carry their positional `seasonNumber`/`episodeNumber` (needed so episode-level seen/review resolve at migration time); provider repopulation later upserts them in place on the same `(external_id, entity_schema_slug, provider_id)` tuple.
...
- V1 `integration` is renamed to `old_integration` in `rename-tables.ts` (its PK constraint too, since the backing index name would collide) because V2 reuses the `integration` name and the Drizzle `CREATE TABLE integration` has no `IF NOT EXISTS`. `integration-mapping.ts` copies `old_integration` into V2 `integration`, assigns ownership to the `media` plugin, and rewrites V1 `provider_specifics` into V2's generic `provider_specifics` settings record (typed `IntegrationProviderSettings`, a `Record<string, unknown>`, not a discriminated union; the shape of each provider's settings — including its `kind` field — is declared by that provider's `settingsSchema` in its plugin manifest and validated against it). Required settings fields are validated up front and unknown providers fail fast; optional fields are emitted only when present; inserts use `ON CONFLICT (id) DO NOTHING`.
- V1 `notification_platform` is renamed to `old_notification_platform` before Drizzle migrations (including its PK and user index) so the legacy source can be copied separately from V2's `notification_channel` table. `notification-platform-mapping.ts` copies platform rows after users exist, maps the tagged `{t,d}` specifics JSON into the V2 `channelSpecifics.kind` union, and drops the legacy credential-bearing description and event filters. Unknown platforms, variant tags, missing required fields, and malformed specifics fail fast; inserts use `ON CONFLICT (id) DO NOTHING`.
- Legacy per-platform event selections are intentionally not migrated. User bootstrap installs the active default notification subscriptions, and each subscription targets all of that user's enabled notification platforms.
