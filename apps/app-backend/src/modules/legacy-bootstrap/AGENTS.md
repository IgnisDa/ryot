...
- V1 `seen` rows become V2 `event` rows. Each `InProgress` row expands into N progress events (one per `updated_at` entry). Non-episodic `Completed` rows and all `Dropped`/`OnAHold` rows expand into N-1 progress events + 1 terminal event (`complete`, `dropped`, or `on_hold`). Episodic `Completed` rows (`show`, `anime`, `manga`, `podcast`) expand into N progress events, with the final event forced to `progressPercent = 100` for that episode/chapter; a second pass emits one whole-entity `complete` event per detected full coverage cycle and then resets coverage for the next watch-through. `InProgress` rows with `progress ≥ 100` are treated as `Completed`. `progress = 0` is clamped to 1. `progress > 100` is clamped to 100. `manual_time_spent` (seconds) is converted to minutes for terminal V2 `timeSpent` when the target event supports it. `completionMode` is always `custom_timestamps` for migrated completions.
