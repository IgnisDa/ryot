...
- After `old_user` is migrated, each legacy user is passed through `bootstrapNewUser` so migrated accounts receive the built-in trackers, saved views, library entity, and active default notification subscriptions that auth-created users get.
...
- V1 `integration` is renamed to `old_integration` in `rename-tables.ts` (its PK constraint too, since the backing index name would collide). Unlike the other V1 tables, the V2 schema reuses the `integration` name, and the Drizzle `CREATE TABLE integration` has no `IF NOT EXISTS`, so the V1 table must be moved aside before Drizzle runs. `integration-mapping.ts` then copies `old_integration` into the V2 `integration` table, rebuilding `provider_specifics` as the V2 discriminated union. Required provider fields are validated up front and unknown providers fail fast; optional fields are emitted only when present so the strict V2 schema parses; inserts use `ON CONFLICT (id) DO NOTHING` for restart-safety. The field-level mappings live in `integration-mapping.ts`.
- The reset V2 baseline creates `notification_channel`, so V1 `notification_platform` does not collide and is deliberately left untouched while Drizzle runs. Legacy notification destinations, credentials, disabled state, and configured-event preferences are not migration inputs and are never read or validated. The original table is dropped only after the rest of migration succeeds. Migrated users start with zero channels and install the same active default signal subscriptions as new users through `bootstrapNewUser`.
