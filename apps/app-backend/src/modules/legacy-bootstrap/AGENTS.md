...
- Migrated user language provider preference sources are normalized from V1 snake_case to V2 kebab-case with an underscore-to-hyphen transform in `user-auth-mapping.ts`.
- Audible language remains a marketplace and identity concern, not a translation overlay: changing marketplace can change the ASIN and therefore the target entity.
