...
- V1 show/podcast season and episode blobs are exploded into V2 `show-season`, `show-episode`, and `podcast-episode` entities by `episodic-sub-entity-mapping.ts` immediately after parent metadata entities are migrated and before review/seen events. The step also writes the parent-child relationships and session-local episode resolution temp tables reused by review/seen migration.
...
- V1 `integration` is renamed to `old_integration` in `rename-tables.ts` (its PK constraint too, since the backing index name would collide). Unlike the other V1 tables, the V2 schema reuses the `integration` name, and the Drizzle `CREATE TABLE integration` has no `IF NOT EXISTS`, so the V1 table must be moved aside before Drizzle runs. `integration-mapping.ts` then copies `old_integration` into the V2 `integration` table. The id, lot, provider and name columns are copied verbatim (already snake-case in V1); `sync_to_owned_collection` becomes `syncOwnership`; `extra_settings.disable_on_continuous_errors` becomes `extraSettings.disableOnContinuousErrors`; and `provider_specifics` is rebuilt as the V2 discriminated union (flat per-provider snake-case keys become camelCase, keyed on a `kind` field equal to the provider, with the radarr and sonarr profile ids becoming a string `profileId`). Required provider fields are validated up front (fail fast); optional fields (usernames, tag ids, disabled sites, password, metadata provider) are only emitted when present so the strict V2 schema parses. Unknown providers fail fast. Inserts use `ON CONFLICT (id) DO NOTHING` for restart-safety.
- V1 `notification_platform` is renamed to `old_notification_platform` before Drizzle migrations (including its PK and user index) because V2 creates the same table name. `notification-platform-mapping.ts` copies platform rows after users exist, preserves IDs/users/timestamps/configured event literals, maps tagged `{t,d}` specifics JSON into the V2 `platformSpecifics.kind` union, omits optional null values, and drops the legacy credential-bearing description. Unknown platforms, event literals, variant tags, missing required fields, and malformed specifics fail fast. Inserts use `ON CONFLICT (id) DO NOTHING` for restart-safety.
- The V1 default `Owned` collection is migrated as a normal V2 collection (entity + `member-of` relationships) by `collection-mapping.ts`, and `buildOwnedCollectionOwnershipMigrationSql` additionally marks each owned entity's existing `in-library` relationship with `{ owned: true, ownershipSources: ["legacy"], ownershipSyncedAt: <now ISO> }` — mirroring the runtime ownership shape. It runs after `user-to-entity-mapping.ts` so the `in-library` relationships already exist; it is an `UPDATE` only (no upsert) because every owned media entity is global and therefore already in-library.
- The V1 default `Monitoring` collection is retained as a normal V2 collection and its global, provider-backed media/person memberships additionally become `media-monitoring` relationships to the matching user's bootstrap-created library entity. `buildMonitoringCollectionMigrationSql` runs after the `in-library` and ownership migrations; it filters to V2-monitorable schemas, preserves the V1 membership timestamp, and uses `ON CONFLICT DO NOTHING` for restart-safety.
...
- Audible marketplace is an identity concern (it changes the ASIN and therefore the target entity), never a translation overlay. It is also no longer a user preference: the Audible provider scripts use a fixed `audible.com` marketplace, so no Audible preference is migrated.

## Ignored For Now

**user_measurement**: `information.assets` images are not migrated to V2 (the `measurement` schema has no `images` property).
...
**review**: `visibility` (V2 has no visibility concept), `comments` (V2 has no comments on events). Show/podcast reviews with positional episode data resolve to episode entities; unresolved show/podcast episode reviews are skipped rather than attached to the parent. See `review-mapping.ts` for rating clamping and NULL-rating handling details.
