...
import { createdAtRef, nameRef, propertyRef } from "./validator.test-support";
...
describe("relationship root sources", () => {
	it("accepts a relationship root source with both endpoint declarations", () => {
		expect(validateQueryDocument(makeRelationshipDoc())).toBeNull();
	});
	...
	it("rejects an invalid system field for the relationship alias", () => {
		...
		expect(validateQueryDocument(doc)).toMatch(/Invalid system field 'name' for relationship edge/);
	});
	...
	it("rejects a relationship property field whose schema is not in the relationship schemas", () => {
		...
		expect(validateQueryDocument(doc)).toMatch(
	...
	it("rejects duplicate relationship schema slugs", () => {
		...
		expect(validateQueryDocument(doc)).toMatch(/Duplicate schema 'member-of'/);
	});
	...
	it("rejects a duplicate alias between the relationship and an endpoint entity", () => {
		...
		expect(validateQueryDocument(doc)).toMatch(/Duplicate alias 'membership'/);
	});
	...
	it("rejects include on relationship root rows", () => {
		...
		expect(validateQueryDocument(doc)).toMatch(/Relationship root rows do not support include yet/);
	});
});
