...
export const evalFieldSelector = (field: FieldSelector, row: BaseEntityQueryRow): FieldValue => {
	...
	if (field.name === "isBuiltin") {
		return { kind: "boolean", value: row.schemaIsBuiltin };
	}
...
export const evalRelationshipFieldSelector = (
	...
	return {
		kind: field.name === "isBuiltin" ? "boolean" : "text",
		value:
			field.name === "slug"
				? row.relationshipSchemaSlug
				: field.name === "isBuiltin"
					? row.relationshipSchemaIsBuiltin
					: row.relationshipSchemaName,
	};
};
