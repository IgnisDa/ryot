...
describe("mapAggregateValue", () => {
	it("returns kind 'number' for count with a positive value", () => {
		expect(
			mapAggregateValue({ key: "total", value: 5, type: "count" }),
		).toEqual({ key: "total", kind: "number", value: 5 });
	});

	it("returns kind 'number' for count with value 0 (empty filtered set)", () => {
		expect(
			mapAggregateValue({ key: "total", value: 0, type: "count" }),
		).toEqual({ key: "total", kind: "number", value: 0 });
	});

	it("returns kind 'number' for countWhere with a positive value", () => {
		expect(
			mapAggregateValue({ key: "recent", value: 3, type: "countWhere" }),
		).toEqual({ key: "recent", kind: "number", value: 3 });
	});
	...
	it("returns empty object for countBy when value is null (empty filtered set)", () => {
		expect(
			mapAggregateValue({ key: "bySchema", value: null, type: "countBy" }),
		).toEqual({ key: "bySchema", kind: "json", value: {} });
	});
