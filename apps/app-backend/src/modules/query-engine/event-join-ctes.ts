...
export const buildLatestEventJoinCte = (input: {
	...
	const eventSchemaIdList = sql.join(
		input.join.eventSchemas.map((s) => sql`${s.id}`),
		sql`, `,
	);

	return sql`
		${sql.raw(getEventJoinCteName(input.join.key))} as (
			select distinct on (${schema.event.entityId})
			...
			where ${schema.event.userId} = ${input.userId}
				and ${schema.event.eventSchemaId} in (${eventSchemaIdList})
			order by ${schema.event.entityId}, ${schema.event.occurredAt} desc, ${schema.event.createdAt} desc, ${schema.event.id} desc
		)
	`;
};
