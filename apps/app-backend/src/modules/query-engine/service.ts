...
export class QueryEngineService extends Context.Service<QueryEngineService>()(
	"QueryEngineService",
	{
		make: Effect.gen(function* () {
			...
			const runBounded = <A>(
				effect: Effect.Effect<A, BadRequest | NotFound | DbError, CurrentDb | DefinitionRegistry>,
			) =>
				runInTx(
					withDefinitions(
						Effect.gen(function* () {
			...
			const executeDocument = Effect.fn("QueryEngineService.executeDocument")(function* (
				...
				if (isRowsQueryDocument(doc)) {
					return yield* runWithDb(runBounded(executeRowsQuery(executionScope, language, doc)));
				}

				if (isAggregateQueryDocument(doc)) {
					return yield* runWithDb(runBounded(executeAggregateQuery(executionScope, language, doc)));
				}

				if (isTimeSeriesQueryDocument(doc)) {
					return yield* runWithDb(
						runBounded(executeTimeSeriesQuery(executionScope, language, doc)),
					);
				}
