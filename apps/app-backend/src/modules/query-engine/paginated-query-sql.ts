...
type QueryEngineItem = Readonly<Record<string, ResolvedDisplayValue>>;
...
export const executePaginatedQuery = (input: {
	...
	paginationConfig: Omit<
		...
		| "offset"
	>;
}): Effect.Effect<{ items: QueryEngineItem[]; pagination: PaginationResult }, DbError, CurrentDb> =>
	Effect.gen(function* () {
		const db = yield* CurrentDb;
		const offset = (input.pagination.page - 1) * input.pagination.limit;
		const dataResult = yield* dbEffect(() =>
			db.execute<QueryRow>(
