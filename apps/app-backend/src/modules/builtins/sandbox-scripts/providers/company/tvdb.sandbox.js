...
driver("search", async function (context) {
	...
	const items = results
		.map((item) => {
			...
			const imageUrl = item?.poster ?? item?.image_url ?? item?.primaryImage ?? null;
...
driver("details", async function (context) {
	...
	const data = await tvdbGet(`/companies/${externalId}`);
	...
	const images = [];
	const primaryImage =
		typeof company?.primaryImage === "string" && company.primaryImage.trim()
			? company.primaryImage.trim()
			: null;
	if (primaryImage) {
