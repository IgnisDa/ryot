...
driver("search", async function (context) {
	...
	const items = results
		.map((company) => {
			...
			const externalId =
				typeof company.guid === "string" && company.guid.trim() ? company.guid.trim() : null;
			if (!externalId) {
				return null;
			}
...
driver("details", async function (context) {
	...
	const company = payload?.results && typeof payload.results === "object" ? payload.results : null;

	if (!company) {
		throw new Error("GiantBomb returned no company data");
	}

	const name = typeof company.name === "string" && company.name.trim() ? company.name.trim() : null;
	...
	return {
		...
		properties: {
			...
			sourceUrl:
				typeof company.site_detail_url === "string" && company.site_detail_url.trim()
