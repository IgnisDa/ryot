...
async function getMetronCredentials() {
	...
	const username = typeof usernameResponse.data === "string" ? usernameResponse.data.trim() : null;
	const password = typeof passwordResponse.data === "string" ? passwordResponse.data.trim() : null;
...
async function metronGet(path) {
	const { username, password } = await getMetronCredentials();
	const response = await httpCall("GET", `${METRON_BASE_URL}${path}`, {
		headers: { Authorization: `Basic ${btoa(`${username}:${password}`)}` },
	});
	if (!response?.success) {
		throw new Error(response?.error ?? `Metron request failed: ${path}`);
	}
...
driver("search", async function (context) {
	...
	const data = await metronGet(`/series/?${params.toString()}`);
	...
	const nextPage = data?.next != null ? currentPage + 1 : null;
...
driver("details", async function (context) {
	...
	if (!/^\d+$/.test(externalId)) {
		throw new Error("externalId must be a numeric Metron series ID");
	}
	...
	const parts = typeof series?.issue_count === "number" ? series.issue_count : 0;

	// Fetch the issue list for this series to get member items with positions.
	const issueListData = await metronGet(`/series/${externalId}/issue_list/?limit=100`);
	const issueList = Array.isArray(issueListData?.results) ? issueListData.results : [];
	...
	return {
		...
		properties: {
			...
			images: [],
			description,
			sourceUrl: `https://metron.cloud/series/${externalId}`,
		},
	};
});
