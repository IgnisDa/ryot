...
driver("search", async function (context) {
	...
	const { query, page: currentPage } = z
		.object({
	...
	const totalItems = typeof artists?.total === "number" ? artists.total : 0;
	const artistItems = Array.isArray(artists?.items) ? artists.items : [];

	const items = artistItems
		.map((artist) => {
			const externalId = getString(artist?.id);
			if (!externalId) {
				return null;
			}
	...
	return {
		...
		details: {
			totalItems,
			nextPage: offset + artistItems.length < totalItems ? currentPage + 1 : null,
		},
	};
});
...
driver("details", async function (context) {
	...
	const artist = await spotifyGet(`/artists/${encodeURIComponent(externalId)}`);
	...
	const relatedEntities = await (async () => {
		...
		while (true) {
			const data = await spotifyGet(`/artists/${encodeURIComponent(externalId)}/albums`, {
	...
	return {
		...
		properties: {
			...
			description,
			alternateNames: [],
			images: getImagesSortedBySize(images).map((url) => ({ type: "remote", url })),
		},
	};
});
