...
driver("search", async function (context) {
	...
	const items = creators
		.map((creator) => {
			const externalId = getIdentifier(creator?.id);
			...
			const birthYear = parseYear(creator?.birth, dayjs);
...
driver("details", async function (context) {
	...
	const payload = await loadMetronJson(
		`https://metron.cloud/api/creator/${encodeURIComponent(externalId)}/`,
	...
	return {
		...
		properties: {
			...
			sourceUrl: `https://metron.cloud/creator/${externalId}`,
		},
	};
});
