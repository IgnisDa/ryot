...
export const buildArtistSearch = (client: MusicSearchClient, query: string) =>
	client.music.search(query, { type: "artist" }).then((results) => {
...
export const buildArtistDetails = (client: ArtistClient, externalId: string) =>
	client.music.getArtist(externalId).then((artist) => {
...
export const search = defineProviderDriver(manifest, "search", (input, host) =>
	createYoutubeMusicClient(host).then((client) => buildArtistSearch(client, input.query)),
);

export const details = defineProviderDriver(manifest, "details", (input, host) =>
	createYoutubeMusicClient(host, manifest.providerInformation.canonicalLanguage).then((client) =>
		buildArtistDetails(client, input.externalId),
	),
);
...
export const translate = defineProviderDriver(manifest, "translate", (input, host) =>
	createYoutubeMusicClient(host, input.language).then((client) =>
		buildArtistTranslate(client, input.externalId),
	),
);
