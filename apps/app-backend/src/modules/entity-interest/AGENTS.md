...
- **[messages.ts](messages.ts)** — the wire schemas (Effect `Schema`): `EntityUpdatedReason` (the completion-reason vocabulary, owned here), `EntityUpdatedMessage`/`encodeEntityUpdatedMessage`/`decodeEntityUpdatedMessage` (the Redis pub/sub payload shape and its codec), `EntityUpdatedFrame` (reused for both the SSE payload and each `terminal` entry), `ConnectedFrame`, `DeclareInterestBody`, `DeclareInterestResponse`, plus the `MAX_INTEREST_ENTITY_IDS = 500` cap constant.
...
- **[service.ts](service.ts)** — `InterestReconciler`: builds the query-engine document, reads the interest set (chunked at `MAX_ROOT_PAGE_SIZE = 100`), and in `handleRow` maps each row to an enqueue-or-terminal decision (the populate-before-translate state machine).
- **[routes.ts](routes.ts)** — the two thin handlers wiring the above together: register-before-reconcile, cap + log, swallow reconcile failure, gate terminal results on current interest.
- **[stream.test.ts](stream.test.ts)** — unit test for the SSE lifecycle: `add` on subscribe + `connected` frame; `remove` on interrupt. Uses a stand-in registry so no Redis is needed.
...
- **Read-path-coupled work.** Previously `getById` (and query-engine reads) triggered population and applied a translation overlay as a _side effect of reading_. That made GETs non-idempotent and coupled the read contract to queue mechanics. Reads are now strictly side-effect-free; the read-time overlay machinery (`entities/translation-overlay.ts`, `translations/translation-overlay-live.ts`, `TranslationsService.resolveOverlay`) was deleted. All work is now driven only by declared interest.
- **A WebSocket gateway** (`/api/ws`). The socket's only real job was carrying the client→server interest channel, which a plain POST does equally well, paired with SSE for the server→client stream. Both are ordinary HTTP that reuse the existing auth middleware, drop the bespoke `Origin`/CSRF handling and the subprotocol-token-to-`Cookie` translation, and can live on the same contract. For a self-hosted single-instance backend, a persistent bidirectional socket bought nothing.
