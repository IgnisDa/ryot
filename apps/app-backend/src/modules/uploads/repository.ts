...
export class UploadsRepository extends Context.Service<UploadsRepository>()("UploadsRepository", {
	make: Effect.sync(() => {
		const getByLocator = Effect.fn("UploadsRepository.getByLocator")(function* (
		...
		const registerPermanentOwnedObject = Effect.fn(
			"UploadsRepository.registerPermanentOwnedObject",
			...
			const [inserted] = yield* mapDatabaseErrors(
				db
					...
					.values(input)
					.onConflictDoNothing({ target: [schema.managedAsset.provider, schema.managedAsset.key] })
					.returning(),
			);
			if (inserted) {
		...
		const listByOwner = Effect.fn("UploadsRepository.listByOwner")(function* (ownerUserId: UserId) {
			const db = yield* Database;
		...
		const listByOwnerAndLocators = Effect.fn("UploadsRepository.listByOwnerAndLocators")(function* (
			ownerUserId: UserId,
			locators: ReadonlyArray<ManagedAssetLocator>,
		) {
