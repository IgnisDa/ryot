...
import type { FieldValue, TimeSeriesResponseV2 } from "../language";
import { evalAggregateMeasure, evalExprAsBoolean, evalExprValue } from "./expr";
import { executeRootSourceMatches } from "./source-matches";
import type { SourceMatch, TimeSeriesQueryDocumentV2 } from "./types";

type TimeSeriesBucket = TimeSeriesQueryDocumentV2["output"]["time"]["bucket"];
...
export const executeTimeSeriesQuery = (
	userId: string,
	doc: TimeSeriesQueryDocumentV2,
): Effect.Effect<TimeSeriesResponseV2, BadRequest | NotFound | DbError, CurrentDb> =>
	Effect.gen(function* () {
		...
		const buckets: Array<TimeSeriesResponseV2["data"]["buckets"][number]> = [];
