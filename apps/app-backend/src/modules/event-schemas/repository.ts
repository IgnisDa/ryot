...
export class EventSchemasRepository extends Effect.Service<EventSchemasRepository>()(
	...
	{
		sync: () => {
			const getEntitySchemaScopeById = Effect.fn("EventSchemasRepository.getEntitySchemaScopeById")(
			...
			const getScopeForUser = Effect.fn("EventSchemasRepository.getScopeForUser")(
				function* (input: { eventSchemaId: EventSchemaId; userId: UserId }) {
					const db = yield* CurrentDb;
					const [row] = yield* dbEffect(() =>
						db
			...
			const getBuiltinBySlug = Effect.fn("EventSchemasRepository.getBuiltinBySlug")(
				function* (input: { entitySchemaId: EntitySchemaId; slug: string }) {
					const db = yield* CurrentDb;
					const [row] = yield* dbEffect(() =>
						db
							.select({
			...
			const createEventSchema = Effect.fn("EventSchemasRepository.createEventSchema")(
				function* (input: {
					name: string;
					slug: string;
					userId: UserId;
			...
			const findBySlugForUser = Effect.fn("EventSchemasRepository.findBySlugForUser")(
				function* (input: { entitySchemaId: EntitySchemaId; slug: string; userId: UserId }) {
					const db = yield* CurrentDb;
					const [row] = yield* dbEffect(() =>
						db
			...
			const listByEntitySchemaForUser = Effect.fn(
