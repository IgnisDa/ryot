...
const mockEpisodeResolverRepository = Layer.mock(EpisodeResolverRepository);

const makeRepository = (ids: ReadonlyArray<EntityId>, podcastIds: ReadonlyArray<EntityId> = []) =>
	mockEpisodeResolverRepository({
		_tag: "EpisodeResolverRepository",
...
const makeLayer = (repository: ReturnType<typeof makeRepository>) =>
	EpisodeResolverService.Default.pipe(Layer.provide(Layer.mergeAll(dbRunnerLayer, repository)));
