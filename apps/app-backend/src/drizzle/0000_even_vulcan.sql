...
--> statement-breakpoint
CREATE TABLE "automation_correlation_budget" (
...
--> statement-breakpoint
CREATE TABLE "automation_effect" (
	"id" text PRIMARY KEY NOT NULL,
	"effect_key" text NOT NULL,
	"input_hash" text NOT NULL,
	"downstream_execution_id" text,
	"host_function" text NOT NULL,
	"result" jsonb,
	"correlation_units" integer DEFAULT 0 NOT NULL,
	"status" text NOT NULL,
	"created_at" timestamp with time zone DEFAULT now() NOT NULL,
	"correlation_id" text NOT NULL,
	"parent_run_id" text NOT NULL,
	"updated_at" timestamp with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "automation_effect_status_check" CHECK ("automation_effect"."status" in ('pending', 'accepted', 'failed'))
);
--> statement-breakpoint
CREATE TABLE "automation_rule" (
	...
	CONSTRAINT "automation_rule_exactly_one_target_check" CHECK (num_nonnulls("automation_rule"."entity_schema_id", "automation_rule"."event_schema_id", "automation_rule"."relationship_schema_id", "automation_rule"."signal_schema_id") = 1),
	CONSTRAINT "automation_rule_target_operation_check" CHECK ((
					("automation_rule"."signal_schema_id" is not null and "automation_rule"."kind" = 'subscription' and "automation_rule"."operation" = 'signal')
					or
					("automation_rule"."signal_schema_id" is null and "automation_rule"."operation" <> 'signal')
				))
);
--> statement-breakpoint
CREATE TABLE "entity" (
...
CREATE TABLE "entity_schema_sandbox_script" (
	...
	"id" text PRIMARY KEY NOT NULL,
	"updated_at" timestamp with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "entity_schema_sandbox_script_unique" UNIQUE("entity_schema_id","sandbox_script_id")
);
--> statement-breakpoint
...
--> statement-breakpoint
CREATE TABLE "sandbox_artifact" (
	"id" text PRIMARY KEY NOT NULL,
	"execution_id" text NOT NULL,
	"value" jsonb NOT NULL,
	"created_at" timestamp with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "sandbox_artifact_execution_id_unique" UNIQUE("execution_id")
);
--> statement-breakpoint
...
CREATE TABLE "signal_recipient" (
	"signal_created_at" timestamp (3) with time zone NOT NULL,
	"user_id" text NOT NULL,
	"signal_id" text NOT NULL,
	"signal_schema_id" text NOT NULL,
	CONSTRAINT "signal_recipient_signal_user_unique" UNIQUE("signal_id","user_id")
);
--> statement-breakpoint
CREATE TABLE "signal_schema" (
	...
	"created_at" timestamp with time zone DEFAULT now() NOT NULL,
	"user_id" text,
	"catalog_state" text DEFAULT 'hidden' NOT NULL,
	"id" text PRIMARY KEY NOT NULL,
	"updated_at" timestamp with time zone DEFAULT now() NOT NULL,
...
CREATE TABLE "subscription_run" (
	...
	"rule_id" text,
	"status" text NOT NULL,
	CONSTRAINT "subscription_run_source_check" CHECK (("subscription_run"."signal_id" is not null and "subscription_run"."operation" = 'signal' and "subscription_run"."lifecycle_occurrence_id" is null)
				or ("subscription_run"."signal_id" is null and "subscription_run"."operation" <> 'signal' and "subscription_run"."lifecycle_occurrence_id" is not null and "subscription_run"."source_kind" is not null and "subscription_run"."record_id" is not null))
);
--> statement-breakpoint
...
ALTER TABLE "automation_effect" ADD CONSTRAINT "automation_effect_correlation_id_automation_correlation_budget_correlation_id_fk" FOREIGN KEY ("correlation_id") REFERENCES "public"."automation_correlation_budget"("correlation_id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
ALTER TABLE "automation_effect" ADD CONSTRAINT "automation_effect_parent_run_id_subscription_run_id_fk" FOREIGN KEY ("parent_run_id") REFERENCES "public"."subscription_run"("id") ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
...
ALTER TABLE "signal_recipient" ADD CONSTRAINT "signal_recipient_signal_schema_id_signal_schema_id_fk" FOREIGN KEY ("signal_schema_id") REFERENCES "public"."signal_schema"("id") ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
...
CREATE INDEX "automation_effect_parent_run_id_idx" ON "automation_effect" USING btree ("parent_run_id");--> statement-breakpoint
CREATE INDEX "automation_effect_correlation_id_idx" ON "automation_effect" USING btree ("correlation_id");--> statement-breakpoint
...
CREATE UNIQUE INDEX "automation_rule_user_entity_unique" ON "automation_rule" USING btree ("user_id","entity_schema_id","operation","sandbox_script_id") WHERE "automation_rule"."entity_schema_id" is not null and "automation_rule"."user_id" is not null;--> statement-breakpoint
CREATE UNIQUE INDEX "automation_rule_user_event_unique" ON "automation_rule" USING btree ("user_id","event_schema_id","operation","sandbox_script_id") WHERE "automation_rule"."event_schema_id" is not null and "automation_rule"."user_id" is not null;--> statement-breakpoint
CREATE UNIQUE INDEX "automation_rule_user_signal_unique" ON "automation_rule" USING btree ("user_id","signal_schema_id","operation","sandbox_script_id") WHERE "automation_rule"."signal_schema_id" is not null and "automation_rule"."user_id" is not null;--> statement-breakpoint
CREATE UNIQUE INDEX "automation_rule_user_relationship_unique" ON "automation_rule" USING btree ("user_id","relationship_schema_id","operation","sandbox_script_id") WHERE "automation_rule"."relationship_schema_id" is not null and "automation_rule"."user_id" is not null;--> statement-breakpoint
CREATE UNIQUE INDEX "automation_rule_builtin_entity_unique" ON "automation_rule" USING btree ("entity_schema_id","operation","sandbox_script_id") WHERE "automation_rule"."entity_schema_id" is not null and "automation_rule"."user_id" is null;--> statement-breakpoint
CREATE UNIQUE INDEX "automation_rule_builtin_event_unique" ON "automation_rule" USING btree ("event_schema_id","operation","sandbox_script_id") WHERE "automation_rule"."event_schema_id" is not null and "automation_rule"."user_id" is null;--> statement-breakpoint
CREATE UNIQUE INDEX "automation_rule_builtin_signal_unique" ON "automation_rule" USING btree ("signal_schema_id","operation","sandbox_script_id") WHERE "automation_rule"."signal_schema_id" is not null and "automation_rule"."user_id" is null;--> statement-breakpoint
CREATE UNIQUE INDEX "automation_rule_builtin_relationship_unique" ON "automation_rule" USING btree ("relationship_schema_id","operation","sandbox_script_id") WHERE "automation_rule"."relationship_schema_id" is not null and "automation_rule"."user_id" is null;--> statement-breakpoint
...
CREATE INDEX "signal_recipient_user_created_signal_idx" ON "signal_recipient" USING btree ("user_id","signal_created_at" DESC NULLS LAST,"signal_id" DESC NULLS LAST);--> statement-breakpoint
CREATE INDEX "signal_recipient_user_schema_created_signal_idx" ON "signal_recipient" USING btree ("user_id","signal_schema_id","signal_created_at" DESC NULLS LAST,"signal_id" DESC NULLS LAST);--> statement-breakpoint
CREATE INDEX "signal_schema_user_id_idx" ON "signal_schema" USING btree ("user_id");--> statement-breakpoint
CREATE UNIQUE INDEX "signal_schema_builtin_slug_unique" ON "signal_schema" USING btree ("slug") WHERE "signal_schema"."user_id" is null;--> statement-breakpoint
CREATE INDEX "subscription_run_execution_user_queued_id_idx" ON "subscription_run" USING btree ("execution_user_id","queued_at" DESC NULLS LAST,"id" DESC NULLS LAST);--> statement-breakpoint
CREATE INDEX "subscription_run_original_rule_id_idx" ON "subscription_run" USING btree ("original_rule_id");--> statement-breakpoint
CREATE INDEX "subscription_run_status_idx" ON "subscription_run" USING btree ("status");--> statement-breakpoint
