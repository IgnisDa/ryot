...
- Built-ins not yet migrated remain `.sandbox.js` function-body fragments. Seeding wraps those entries in a temporary format-0 ES module, while migrated `.sandbox.ts` entries are compiled into the ignored `generated-sandbox` registry as format 1. The Deno runner imports both formats without dynamic function construction.
