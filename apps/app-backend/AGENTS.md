...
- Plugin and legacy per-user scripts compile to format-1 JavaScript; execution imports the compiled module in Deno, never the stored source. Plugin scripts are source-canonical, immutable per content hash, and resolved from the active loader snapshot. There is no legacy `.sandbox.js` fragment, `code` column, or format-0 path.
