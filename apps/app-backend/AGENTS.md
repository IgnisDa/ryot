...
- Plugin and legacy per-user scripts compile to format-1 JavaScript; execution imports the compiled module in Deno, never the stored source. Plugin scripts are source-canonical, immutable per content hash, and resolved from the active loader snapshot. Kernel source-zero scripts use the same content-addressed execution path without belonging to a synthetic plugin. There is no legacy `.sandbox.js` fragment, `code` column, or format-0 path.
- The generated runner and runner-test registry are regenerated by `check`, `test`, `build`, and `dev`. Production plugin loading uses persisted ingestion outputs, not the generated test registry. See `src/lib/infrastructure/sandbox-runtime/README.md` for the compilation pipeline and runtime mechanics.
