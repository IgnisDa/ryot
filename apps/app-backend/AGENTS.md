...
- The generated registry and runner are regenerated by `check`, `test`, `build`, and `dev` so they cannot go stale. See `src/lib/infrastructure/sandbox-runtime/README.md` for the compilation pipeline and runtime mechanics.
