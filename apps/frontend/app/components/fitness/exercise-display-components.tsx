...
export const DisplayData = (props: { name: string; data?: string | null; noCasing?: boolean }) => {
	return (
		<Box>
...
export const DisplayLifetimeStatistic = (props: { val: string | number; stat: string }) => {
		...
		<Group justify="space-between" key={`${props.set.workoutId}-${props.set.setIdx}`}>
			<Text size="sm">
				{match(props.personalBestLot)
					.with(WorkoutSetPersonalBest.OneRm, () => Number(set.statistic.oneRm).toFixed(2))
