...
import { useApplicationEvents, useMetadataDetails, useUserPreferences } from "~/lib/shared/hooks";
import {
...
export const ReviewEntityForm = (props: { closeReviewEntityModal: () => void }) => {
	...
	>({
		...
		initialValues: {
			...
			mangaVolumeNumber: entityToReview?.existingReview?.mangaExtraInformation?.volume,
			animeEpisodeNumber: entityToReview?.existingReview?.animeExtraInformation?.episode,
			mangaChapterNumber: entityToReview?.existingReview?.mangaExtraInformation?.chapter,
			showSeasonNumber: entityToReview?.existingReview?.showExtraInformation?.season?.toString(),
			showEpisodeNumber: entityToReview?.existingReview?.showExtraInformation?.episode?.toString(),
			...
			ratingInThreePointSmiley: entityToReview?.existingReview?.rating
				? convertDecimalToThreePointSmiley(Number(entityToReview.existingReview.rating))
				: undefined,
		},
	});
	...
	const SmileySurround = (props: { children: ReactNode; smileyRating: ThreePointSmileyRating }) => (
		<ThemeIcon
			size="xl"
			style={{ cursor: "pointer" }}
			variant={
				props.smileyRating === form.values.ratingInThreePointSmiley ? "outline" : "transparent"
			}
			onClick={() => {
	...
	return (
		<form
			onSubmit={form.onSubmit((values) => {
				...
				const {
					ratingInThreePointSmiley: _ratingInThreePointSmiley,
					showSeasonNumber,
					showEpisodeNumber,
					podcastEpisodeNumber,
					...rest
				...
				const input: CreateOrUpdateReviewInput = {
					...rest,
					showSeasonNumber: showSeasonNumber ? Number(showSeasonNumber) : undefined,
					showEpisodeNumber: showEpisodeNumber ? Number(showEpisodeNumber) : undefined,
					podcastEpisodeNumber: podcastEpisodeNumber ? Number(podcastEpisodeNumber) : undefined,
				};
			...
			<Stack>
					...
					{match(userPreferences.general.reviewScale)
						.with(UserReviewScale.OutOfFive, () => (
							<Flex gap="sm" mt="lg">
								...
								<Rating
									fractions={2}
									value={form.values.rating ? Number(form.values.rating) : undefined}
									onChange={(v) => form.setFieldValue("rating", v ? v.toString() : undefined)}
								/>
							</Flex>
						))
							...
							<Stack gap={4}>
								...
								<Group justify="space-around" wrap="nowrap">
									<SmileySurround smileyRating={ThreePointSmileyRating.Happy}>
										<Text size="xl">{getThreePointSmileyEmoji(ThreePointSmileyRating.Happy)}</Text>
									</SmileySurround>
									...
									<SmileySurround smileyRating={ThreePointSmileyRating.Sad}>
										<Text size="xl">{getThreePointSmileyEmoji(ThreePointSmileyRating.Sad)}</Text>
									</SmileySurround>
								</Group>
							</Stack>
						))
					...
					<Stack gap={4}>
						<Select
							...
							onChange={(v) => {
								...
								form.setFieldValue("showEpisodeNumber", undefined as string | undefined);
							}}
						...
						<Select
							...
							onChange={(v) => {
								form.setFieldValue("showEpisodeNumber", v as string | undefined);
							}}
							data={
								metadataDetails?.showSpecifics?.seasons
									.find((s) => s.seasonNumber.toString() === form.values.showSeasonNumber)
									?.episodes.map((e) => ({
				...
				{entityToReview.metadataLot === MediaLot.Podcast ? (
					<Select
						...
						onChange={(v) => {
							form.setFieldValue("podcastEpisodeNumber", v as string | undefined);
						}}
				...
				{entityToReview.metadataLot === MediaLot.Anime ? (
					<NumberInput
						...
						onChange={(v) => form.setFieldValue("animeEpisodeNumber", v as number)}
					/>
				) : null}
				{entityToReview.metadataLot === MediaLot.Manga ? (
					<Group wrap="nowrap">
						<NumberInput
							...
							label="Chapter"
							value={
								form.values.mangaChapterNumber ? Number(form.values.mangaChapterNumber) : undefined
							}
							onChange={(v) => form.setFieldValue("mangaChapterNumber", v?.toString())}
						/>
						...
						<NumberInput
							...
							onChange={(v) => form.setFieldValue("mangaVolumeNumber", v as number)}
						/>
					</Group>
				) : null}
				<Textarea
				...
				<Button mt="md" w="100%" type="submit" loading={reviewMutation.isPending}>
