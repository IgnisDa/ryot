...
export const RecommendationsSection = (props: { collectionId: string }) => {
	const { filters: search, updateFilters } = useFiltersState(defaultRecommendationsState);
	...
	const { data: recommendations } = useQuery({
		queryKey: queryFactory.collections.collectionRecommendations(input).queryKey,
		queryFn: () => clientGqlService.request(CollectionRecommendationsDocument, { input }),
	});
	...
	return (
		<Stack gap="xs">
			<DebouncedSearchInput
				...
				onChange={(query) => updateFilters({ recommendationsPage: 1, recommendationsQuery: query })}
			/>
			{recommendations ? (
					...
					<>
						<ApplicationGrid>
							{recommendations.collectionRecommendations.items.map((r) => (
								<MetadataDisplayItem key={r} metadataId={r} shouldHighlightNameIfInteracted />
							))}
						</ApplicationGrid>
						<ApplicationPagination
							value={search.recommendationsPage}
							onChange={(page) => updateFilters({ recommendationsPage: page })}
							totalItems={recommendations.collectionRecommendations.details.totalItems}
						/>
					</>
				) : (
