...
const formatBaseEntityDisplayItemRating = (rating: number, scale: UserReviewScale): string => {
	switch (scale) {
...
const BaseEntityDisplayItemReason = (props: { reason: UserToMediaReason }) => {
	const [Icon, color] = match(props.reason)
		.with(UserToMediaReason.Owned, () => [IconBackpack, "var(--mantine-color-blue-4)"] as const)
		.with(
	...
	return (
		<Tooltip label={changeCase(props.reason)}>
			<Flex w={24} h={24} align="center" justify="center" style={overlayIconBoxStyle}>
...
const EntityActionButton = (props: {
	...
	<Tooltip label={props.label}>
		...
		>
			<ActionIcon onClick={props.onClick} className={props.className} {...props.entityButtonProps}>
				<props.icon size={20} color={getThemeColor(props.colorName, props.mode)} />
			</ActionIcon>
		</Indicator>
	</Tooltip>
);
...
const ActionButtons = memo((props: ActionButtonsProps) => {
	const entityButtonProps = useMemo<ActionIconProps>(() => ({ size: 28, variant: "default" }), []);
		...
		<>
			{props.interactionButtons.includes("consume") && (
				<EntityActionButton
					...
					className={
						props.isFirstItem ? OnboardingTourStepTarget.OpenMetadataProgressForm : undefined
					}
				/>
			)}
...
const BaseEntityDisplayItemComponent = forwardRef<HTMLDivElement, BaseEntityDisplayItemCard>(
	(props, viewportRef) => {
		...
		const fallback = useFallbackImageUrl(
			props.isDetailsLoading ? "Loading..." : props.title ? getInitials(props.title) : undefined,
		);
		...
		const ratingBadgeStyleValue = useMemo(
			() => ({
				...ratingBadgeStyle,
				fontSize: ratingScale === UserReviewScale.ThreePointSmiley ? "12px" : "10px",
			}),
			[ratingScale],
		);
		...
		const handleToggleWatchlist = useCallback(async () => {
			const mutation = alreadyInWatchlist ? removeEntitiesFromCollection : addEntitiesToCollection;
		...
		}, [props.title, props.mediaLot, props.entityId, props.entityLot, setEntityToReview]);
		...
		return (
			<Card
				...
				h={{ base: 240, "2xl": 320, "3xl": 400, "4xl": 440 }}
				withBorder={!shouldHighlightImage && !props.isCalendarEventWatched}
			>
				{props.centerElement ? (
					<>
						<Overlay />
						<Paper top="50%" left="50%" withBorder pos="absolute" style={centerElementPaperStyle}>
							{props.centerElement}
						</Paper>
					</>
				) : null}
				...
				>
					<Image w="100%" h="100%" fit="cover" src={props.image} fallbackSrc={fallback} />
				</Link>
				<Group pos="absolute" wrap="nowrap" w="100%" gap={2} p={2}>
					...
					{MediaIcon && props.mediaLot ? (
						<Tooltip label={changeCase(props.mediaLot)}>
							<Flex w={24} h={24} align="center" justify="center" style={mediaIconBoxStyle}>
					...
					{props.rating && (
						<Badge ml="auto" size="sm" style={ratingBadgeStyleValue}>
							{formatBaseEntityDisplayItemRating(Number(props.rating), ratingScale)}
						</Badge>
					)}
				</Group>
				<Box
