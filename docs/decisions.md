...
1. The `POST /api/entity-interest` handler (`modules/entity-interest/routes.ts`, alongside the `GET /api/entity-interest/stream` handler in the same `entity-interest` group) updates the `StreamRegistry` interest set for that `streamId` first, so a workflow that publishes mid-reconcile still finds the stream.
