...
- Interest state is in-process and single-instance: a `POST /api/entity-interest` for a `streamId` must reach the process holding the SSE connection for that `streamId`.
