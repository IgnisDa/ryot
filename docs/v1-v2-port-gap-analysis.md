...
V1 is used as a behavior reference only. V2 replaces V1's domain-specific resolvers with a generic `entity_schema → entity → event → relationship` model queried through `modules/ryotql`, so intentional differences are called out inline.
...
| Provider search (`metadata_search`, `people_search`, `metadata_group_search`)                                                                                     | Provider search entrypoints via `entity-schemas/search` + `entities/import`                                                                                                                                                                                                                                                                                                         | Full                                                                                                                                                                        |
| Metadata lookup (`metadata_lookup`)                                                                                                                               | The media plugin's `metadata-lookup` operation (`POST /plugins/media/operations/metadata-lookup`) using TMDB movie/show sandbox searches and the shared title matcher consumed by the browser extension                                                                                                                                                                             | Full for the V1 browser-extension lookup shape                                                                                                                              |
...
| Seen/progress history                                                                                                                                             | Lifecycle events (`backlog`/`progress`/`complete`/`dropped`/`on_hold`/`review`); state derived via RyotQL                                                                                                                                                                                                                                                                     | Create-only — see [Review & progress-event editing](#review--progress-event-editing)                                                                                        |
...
| Media translation (localized title/overview)                                                                                                                      | `modules/entity-translation` (sandbox `translate` driver + `TranslateEntityWorkflow`); localized `name`/`properties` and a `translationStatus` field via `modules/ryotql`; filled on demand through client-declared interest (`modules/entity-interest`) rather than V1's periodic refresh job                                                                                | Functionally equivalent; drops per-variant (title/image/description) granularity and show/podcast episode-scoped translation — low priority                                 |
| Integration progress normalization (clamp above configured maximum to 100%, filter below configured minimum, auto-fill a missing completion timestamp with "now") | Plugin event-create automation policy `trigger.integration-progress-policy` (`plugins/media/scripts/automations/integration-progress-policy.sandbox.ts`); timestamp fallback is structural via `modules/integrations/sinks/shared.ts`'s `createProgressResult`                                                                                                                      | Full — see `plugins/media/AGENTS.md` ("Integration Progress Policy"); regression coverage in `tests/src/tests/plugins/media/integrations/integrations.test.ts` ("Progress normalization") |
| Media & collection recommendations (`user_metadata_recommendations`, `collection_recommendations`)                                                                | `media-suggestion` relationship schema (`plugins/media/schemas/relationship-schemas.ts`) populated by `modules/provider-entities/population.ts`'s `syncEntitySuggestions()`, triggered from `modules/provider-entities/entity-import-workflow.ts`; consumed via ad hoc `modules/ryotql` documents                                                                                     | **Partial** — see [Recommendations — known gaps](#recommendations--known-gaps)                                                                                              |
| Media trending metadata                                                                                                                                           | `media-trending` relationship schema and plugin cron (`plugins/media`) dispatched by `modules/scheduler/plugin-cron`; TMDB movie/show sandbox trending entrypoints persist global media self-edges and clients read them through `modules/ryotql` documents                                                                                                   | Full for V1 provider parity (TMDB movie/show); no dedicated endpoint or saved view                                                                                          |
...
| Media monitoring                                                                                                                                                  | `media-monitoring` relationship schema (`plugins/media/schemas/relationship-schemas.ts`) managed through the media plugin's status/enable/disable invoke operations; its targeted workflow cron pages a system query and runs provider population in refresh mode, while plugin detector automations emit semantic signals to catalog subscriptions scoped by the monitoring relationship | Full for V1 provider-details notification parity                                                                                                                            |
...
The `media-suggestion` relationship graph is populated and both media and collection recommendations can be computed from it (verified by the media RyotQL tests), but two things still separate this from V1:

- **No dedicated endpoint or saved view.** Both recommendation types are only demonstrated via hand-assembled RyotQL documents in the tests above — there is no callable route or built-in saved view a client can hit today.
