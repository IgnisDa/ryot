...
Also do not cite dependency manifests as usage evidence. A real example should have source imports
and actual `Workflow.make` / `Activity.make` / engine wiring, not only a package entry or lockfile
resolution.
...
| Treating ordinary Effect primitives as durable workflow primitives | `livestorejs/livestore` has production-quality durable storage elsewhere, but no `@effect/workflow` source usage at the audited commit; its email-client example performs queue/subscription side effects with in-memory dedupe and `new Date()` payloads ([queue sends](https://github.com/livestorejs/livestore/blob/a3a983039e1547ad8733d5764312d88d796bba0e/examples/web-email-client/src/cf-worker/ThreadClientDO.ts#L77-L128), [dedupe](https://github.com/livestorejs/livestore/blob/a3a983039e1547ad8733d5764312d88d796bba0e/examples/web-email-client/src/cf-worker/ThreadClientDO.ts#L18-L23)). `ComposioHQ/composio` uses ordinary `Effect.retry`, `Deferred`, and `Effect.sleep`, but no workflow source API at the audited commit ([retry](https://github.com/ComposioHQ/composio/blob/914bccf3559bd91b3332d41906784a6c2c2e1bf6/ts/packages/cli/src/commands/login.cmd.ts#L462), [deferred/sleep](https://github.com/ComposioHQ/composio/blob/914bccf3559bd91b3332d41906784a6c2c2e1bf6/ts/packages/cli/src/commands/listen.cmd.ts#L444-L565)). | Regular `Effect.retry`, `Deferred`, `Effect.sleep`, queues, Durable Objects, and event logs may be good tools, but they are not workflow replay/activity memoization. External effects still need deterministic activity keys and durable completion records. |
...
| [`livestorejs/livestore`](https://github.com/livestorejs/livestore/tree/a3a983039e1547ad8733d5764312d88d796bba0e) | `a3a983039e1547ad8733d5764312d88d796bba0e` | Not relevant as a direct workflow example: `@effect/workflow` appears in package metadata, but no source imports or workflow primitives were found. Useful adjacent lesson: `layerMemory` was test-only, real Cloudflare storage was visible, and event-log replay idempotency is not a substitute for activity-level idempotency around external effects. |
| [`ComposioHQ/composio`](https://github.com/ComposioHQ/composio/tree/914bccf3559bd91b3332d41906784a6c2c2e1bf6) | `914bccf3559bd91b3332d41906784a6c2c2e1bf6` | Not relevant as a workflow example: it declares `@effect/workflow` in CLI metadata/lockfile only, with no `Workflow`, `Activity`, engine, durable primitive, or `idempotencyKey` source usage found. Useful adjacent lesson: it explicitly avoids silent retries for non-idempotent writes until backend idempotency exists. |
