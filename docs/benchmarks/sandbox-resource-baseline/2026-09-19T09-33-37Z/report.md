...
- 50 scenario artifacts across seven phase invocations, 2026-09-19 09:56 UTC to 2026-09-20 11:57 UTC.
...
- `soak-live-details` was not run. It submits `live-details`, which executes synchronously and
  ...
  rather than anything the decision questions ask. `soak-control` supplies the non-import soak
  baseline.
...
Every scenario completed. Across all 50 artifacts:

- 1 020 requests submitted, 1 020 completed, **0 failed**, 0 aborted, 0 skipped.
...
executions *inside* each import. Going from one worker to two removes about 37 s of serialised
...
Applying `retention.ts::retentionClassification` to the two soaks, with the manifest's tolerances
(10 % post-GC growth ratio, 100 MiB per 1 000 operations slope ceiling):

| Input | soak-control | soak-hermetic-import |
| --- | --- | --- |
| operations | 100 direct executions | 200 imports |
| fresh idle RSS | 449.9 MiB | 445.3 MiB |
| final recovery RSS | 485.2 MiB | 927.5 MiB |
| post-GC RSS | 491.0 MiB | 927.1 MiB |
| post-GC RSS growth | **+9.1 %** | **+108.2 %** |
| post-GC heap-used growth | **−4.4 %** | **+10.3 %** |
| post-GC external growth | −6.5 % | +19.8 % |
| JSC object count, fresh → post-GC | 1 011 575 → 982 406 | 1 074 991 → 1 072 018 |
| fitted RSS slope | 27.8 MiB / 100 ops | 198.1 MiB / 100 ops |
| projected per 1 000 ops | 278 MiB | 1 981 MiB |
| **classification** | **allocator-high-water** | **heap-retention** |

### soak-control is the readable one
...
The honest caveat: the fitted slope of 27.8 MiB per 100 operations projects 278 MiB per 1 000
operations, which is above the plan's 100 MiB ceiling, and the growth does not fully flatten (waves
6–10 still add about 15 MiB per 100 operations). 100 operations over 2 h 44 m cannot separate a slow
leak from allocator high-water, and most of that time was idle and recovery windows rather than
execution. **A clean answer needs a longer control soak at a
...
6. **Does Bun retain after drain?** Not on the heap. `soak-control` ends 9.1 % above fresh idle in
   RSS with live heap 4.4 % *below* it and 29 169 fewer JSC objects after a forced GC:
   allocator-high-water. The slope test is not clean at 100 operations, so this needs a longer
   control soak.
7. **If RSS grows, what is it?** Allocator reservation, per 6. The import soak's heap-retention
   classification is invalid as a Ryot measurement because it accrued under stall.
...
- Ryot does not retain heap across drained executions.
...
- Whether `soak-control`'s 27.8 MiB per 100 operations is a slow leak or allocator high-water. Needs
  a longer soak.
