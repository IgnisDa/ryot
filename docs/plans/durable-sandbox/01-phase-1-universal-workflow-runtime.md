...
Task 05 completed the backend-owned classification and write audit on 2026-08-06. `activity`
capabilities call an existing service that does not launch durable work. `service workflow`
capabilities run in a deterministic child because their existing owner launches downstream workflows;
they are never placed inside an activity. Filesystem grants remain outside the bridge protocol, with
durable scratch publication owned by the artifact store from Task 04.

| Capability                                                                                                                                                                              | Durable primitive          | Owner and retry-boundary proof                                                                                                                                                                                                            |
| --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `log`, `span`                                                                                                                                                                           | diagnostic                 | Sandbox observability collector; excluded from the business journal, replay-correlated, bounded, and credential-shaped attributes, messages, span names, and console text are redacted.                                                   |
| `httpCall`                                                                                                                                                                              | activity                   | Runtime HTTP service; immediate bounded attempt. Remote mutations are explicitly at-least-once across the response-persistence crash window and receive no automatic business retry.                                                      |
| `getCachedValue`, `getPluginConfig`, `getSystemConfig`, `getUserPreferences`, `getEntitySchemas`, `listEventSchemas`, `listIntegrations`, `getCurrentIntegration`, `executeQueryEngine` | activity                   | Existing owning read services; successes and typed failures are journaled, so replay does not re-read mutable state.                                                                                                                      |
| `setCachedValue`                                                                                                                                                                        | activity                   | Runtime cache service; repeating the same key/value/expiry converges on the same value. TTL renewal in the crash window is accepted cache behavior.                                                                                       |
| `claimPersistentValue`                                                                                                                                                                  | activity                   | Runtime persistent cache service; Redis `SET NX` owns the claim and an unconditional internal envelope stores the public value plus deterministic sandbox call identity, reproducing `claimed: true` after the write/result crash window. |
| `changeUserRelationships`                                                                                                                                                               | activity                   | Relationship service; unique relationship identity plus idempotent create/delete operations make repeated exact input converge without duplicate rows.                                                                                    |
| `upsertGlobalEntities`                                                                                                                                                                  | activity                   | Entity service/repository; provider, schema, and external ID uniqueness makes repeated exact input an upsert of the same rows.                                                                                                            |
| `upsertGlobalRelationships`                                                                                                                                                             | activity                   | Relationship reconciliation service; the selector transaction upserts the requested identity set and removes only identities outside that set, so exact repetition converges.                                                             |
| `createEvents`                                                                                                                                                                          | owning child workflow      | `EventCreateWorkflow`; deterministic child execution ID and deterministic per-item event IDs deduplicate both workflow replay and the database write.                                                                                     |
| `ensureUserEntities`                                                                                                                                                                    | service workflow child     | Entity service plus lifecycle dispatcher; the entity transaction commits before lifecycle workflows start, the sandbox call supplies deterministic occurrence identity, and user/schema uniqueness makes the write idempotent.            |
| `emitSignal`                                                                                                                                                                            | service workflow child     | Signal emission service; signal IDs derive from deterministic execution/schema/discriminator identity and downstream subscription workflow IDs derive from the signal and rule IDs.                                                       |
| `sendNotification`                                                                                                                                                                      | owning child workflow      | `NotificationDeliveryWorkflow`; deterministic sandbox call identity owns workflow start. External channel delivery remains at-least-once at the provider boundary.                                                                        |
| durable scratch/artifact creation                                                                                                                                                       | durable artifact operation | Sandbox artifact store; content is materialized before an opaque workflow-scoped handle is journaled, and deterministic workflow references retain it through suspension.                                                                 |

Crash-window coverage is strategy-based: the sandbox workflow restart test proves an activity-backed
write is not repeated after completion; persistent-claim tests cover write/result recovery; the
durable tracer covers service and event writes plus immediate HTTP across projection reconstruction;
notification/signal owner tests prove deterministic downstream identities; and Task 04 artifact tests
cover publication, retention, and replay-safe handles.
