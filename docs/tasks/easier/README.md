...
**Implemented.** `dispatchImportRun` is the only post-admission dispatch path. It takes the run, the resolved source, and the artifact context that the two entry points produce, then owns workflow pinning, durable source-state storage, workflow enqueue, and rollback in one place. `startFileImportRun` keeps upload claiming, extension validation, and the file-name summary update; `startSourcePayloadImportRun` keeps its single `create`; both then hand off with `uploadIntentIds` empty for the payload path, so the previously file-only upload cleanup now applies uniformly at no cost. The three near-duplicate failure blocks per path collapse into one `failDispatch` (log, `failRun` with the stage-specific `queue-unavailable` operation, reject with `queue-unavailable`/`import-run`) and one `rollback` (cleanup uploads, delete source state, release the pin only when `registrationStatus` is `registered`). The two copies had already drifted: the payload path skipped upload cleanup entirely and released the pin before deleting source state, while the file path did the reverse. Pre-dispatch failures are unchanged and still delete the run — upload-claim, non-local locator, extension, and summary-update failures all run before `dispatchImportRun` — while every failure after pinning starts retains the run in `failed`. New service tests cover file-path enqueue rollback (uploads deleted, source state deleted, pin released, run failed rather than deleted), pin failure (uploads cleaned, no source state written, no release), and a rollback whose pin was `already-registered` (source state deleted, no release).
...
**Implemented.** Option-schema materialization is split out as `materializeSearchOptionsSchema`, which takes the already-resolved `AppSchema` instead of a provider id to re-resolve. `resolveSearchOptionsSchema` keeps its route-facing signature: it calls `resolveSearch` once, returns `null` when the search operation declares no options schema, and otherwise materializes. `search` already holds that resolution, so it now calls the materializer directly and no longer triggers a second `findProviderAvailableToUser` plus `resolveUserSearchScript` pair — a filtered search can no longer observe two different plugin snapshots inside one request, and it can no longer fail provider-availability the second time after passing it the first. The `?? resolved.optionsSchema` fallback at the call site is removed because it was unreachable: the branch runs only when `optionsSchema` is non-null, so the old call could never return `null` there. The cache key now reads the request's `providerId` rather than `provider.id`, which is the same value because `findProviderAvailableToUser` filters on that id. Auxiliary search-options script resolution stays where it was, inside the materializer, and every `UnsupportedProviderOperationError` mapping (`inactive_provider` → not-found, `unsupported_operation` → `search-unsupported`, options failures → `search-options-unavailable`) is unchanged. A new service test counts resolver calls and asserts a filtered dynamic search resolves the provider once, the search script once, and the search-options script once.
...
**Implemented.** `crates/services/notification/templates/generic.html` is now a committed, hand-owned Askama template instead of a CI artifact. It was previously generated by `email export` and copied over the crate's template directory, which is why the bare `templates` entry in `.gitignore` existed; that entry and the two `!ci/helm/**/templates` negations that only existed to rescue Helm from it are removed.

The two template paths were never compatible. The Rust struct renders `{{ generic_message }}`, but `GenericEmail` defaults `message` to `""`, so the exported HTML carries an empty paragraph and no placeholder. Every release build therefore overwrote a working template with one that renders notification emails with an empty body. The committed template keeps the `{{ generic_message }}` placeholder and fixes its logo URL, which still pointed at the pre-rename `libs/assets/` path.

`packages/transactional` loses its `build` and `copy-templates` scripts; `dev` and `check` are unchanged and the React templates stay the source for `apps/app-backend` and `apps/website`, which import the `.tsx` directly and never consumed the static export. The `build-transactional` CI job, its templates artifact upload, `build-backend`'s dependency on it, and the matching `download-artifact` step are removed, so `build-backend` now needs only `pre-workflow-checks`. The three other exported files (`contact-submission`, `login-code`, `purchase-complete`) are deleted from the crate because no Rust `#[template(path = ...)]` references them.
