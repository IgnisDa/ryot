...
Rename the existing backend so the new Effect backend can own the `@ryot/app-backend` package name without duplicate workspace packages. Create the smallest runnable Effect backend shell with process startup, graceful shutdown, `/api/system/health`, `/api/docs` Scalar OpenAPI documentation, `/api/auth/*` delegation placeholder wiring, `/_i/:integrationId` placeholder routing, and static `./client` SPA fallback behavior.
...
- [x] `/api/docs` serves interactive Scalar OpenAPI documentation
