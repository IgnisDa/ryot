...
Review every file touched during this plan and remove anything that is no longer needed or was introduced as scaffolding. Follow the `codebase-cleanup` skill, with special attention to fake-data remnants, duplicate helpers, dead code, unnecessary exports, shallow wrappers, and stale support artifacts. The cleanup is scoped to touched files and directly affected modules, not unrelated opportunistic refactors.
