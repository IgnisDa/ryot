...
- `ryot.data.invokeOperation` requires `input`; a no-input operation sends explicit `null`, and omitted input is invalid. Invalid SDK input fails locally with `PluginOperationError` reason `invalid-input`. A non-JSON operation output is reported as `malformed-result` before it is sent over the bridge.
