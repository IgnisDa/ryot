...
plugin client source in archive
  ...
  -> shared RyotClient Promise capability and React query/mutation surface
...
- A client manifest entry, when present, is exactly `{ entry: "client/index.tsx", apiVersion: 1 }`. The entry is a relative POSIX path under `client/` ending in `.ts` or `.tsx`, with no `./` prefix. No client capabilities field exists until a real capability is implemented and enforced.
...
- `RyotClient` exposes semantic capability APIs through an explicit client/provider boundary, and its asynchronous request APIs return Promises. The canonical taxonomy is `ryot.data.query(recipe)`, `ryot.operations.invoke({ slug, input, output })`, and `ryot.navigation.push/replace`. The kernel uses a direct adapter; one per-session plugin runtime uses a `MessageChannel` adapter. No global mutable bridge is part of the shared client runtime.
- `@ryot/client-sdk/react` exports `RyotProvider`, `useRyot`, `createRyotQuery`, `useRyotQuery`, `createRyotMutation`, and `useRyotMutation`. Query and mutation hooks expose plain React Query-like result state. Internally, each provider/session owns one `@effect/atom-react` registry/cache; query inputs use `Atom.family`, queries revalidate on mount and browser focus, and unobserved atoms have a five-minute idle TTL. This is not TanStack Query and has no compatibility path.
...
- Queries, operations, navigation, and later capabilities use one public `RyotClientError`. Its reasons are exactly `disposed`, `protocol`, `transport`, `invalid-input`, `query-failed`, `operation-failed`, `malformed-result`, and `unsupported-capability`: teardown, malformed bridge/session data or wire `failed` close, communication/posting/network failure, local pre-dispatch input failure, opaque declared backend/platform query or operation execution failure, invalid capability results, and an exposed SDK category missing from the supplied adapter respectively. Expected plugin business/domain outcomes remain successful typed values, the wire value `failed` is not a public SDK error reason, and internal causes, messages, diagnostics, HTTP details, and stack traces never cross the bridge.
- Shared schemas, `RyotClientError`, the runtime lifecycle, pending-call handling, and teardown machinery are fixed shared infrastructure. Theme, crash, and artifact-reload work reuses them and adds no capability-, theme-, crash-, or reload-specific error unions or compatibility paths.
- Each plugin session has one runtime owning its `MessagePort`, `ready`/`active`/`closing`/`failed`/`disposed` state, single dispatcher, location state, query and operation pending calls, listeners, client, and disposal. Operation and RyotQL pending requests share an aggregate maximum of 64 per session, enforced by the SDK and kernel. Exceeding it is a protocol failure using the existing wire `failed` and public `protocol` teardown; requests are not queued or retried, and no new error reason is added. Theme, crash, and artifact-reload work reuse this runtime. All pending calls reject at most once during failure or disposal; kernel abort is best effort and cannot undo committed backend work.
...
- The client catalog returns active plugins only, decodes client API version as exact `1`, follows bounded cursor pages of at most 100 rows to completion, and exposes no capabilities.
- `ryot.data.query(recipe, { signal })` executes a recipe, forwards the optional `AbortSignal` through the direct or `MessageChannel` adapter, and decodes its result locally. It uses the existing user-scoped backend authorization and does not introduce a client-specific authorization bypass.
...
- The client API and artifact-format markers are exact, and the bridge uses exact protocol version 1. The one per-session runtime routes session messages through its single dispatcher, carries strict `{ type: "lifecycle-close", reason: "disposed" | "failed" }` signaling, supports strict `{ type: "ryotql-cancel", requestId }` query cancellation, and rejects pending calls during disposal. Runtime-owned theme messages use that same exact contract.
...
- the complete client SDK or client UI SDK beyond the demonstrated capability and React query/mutation surfaces
- durable plugin client storage
- files, notifications, audio, haptics, screen, or Live Activity capabilities
- manifest-declared client capabilities before a real enforced capability exists
