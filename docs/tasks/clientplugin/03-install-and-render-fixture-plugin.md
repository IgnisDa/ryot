...
- [x] The kernel transfers a `MessagePort` to the intended top-level plugin document, validates exact markers including bridge protocol version 1, and receives a ready signal before presenting the plugin as loaded.
- [x] The fixture bootstraps with `bootstrapClientPlugin({ home, routes })`, uses at least one minimal client UI SDK primitive, and visibly renders its home without a kernel import of fixture UI code.
...
- **Artifact layout.** Multi-file with unique flat single-segment names: `index.html`, `plugin.js`, `plugin.css`, and content-hashed assets such as `asset-<sha256>.svg`. Identical assets with the same extension share one emitted file. `index.html` references the others relatively, so the artifact loads from any prefix without rewriting. The hash covers `plugin.js`, `plugin.css`, and assets but never `index.html`, which carries the resulting hash.
