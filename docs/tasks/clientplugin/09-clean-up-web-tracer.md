...
Audit the final SDK taxonomy and all public names: `ryot.data.query(recipe, { signal })`, `ryot.operations.invoke({ slug, input, output })`, and `ryot.navigation.push/replace`, plus `createRyotQuery`/`useRyotQuery` and `createRyotMutation`/`useRyotMutation` from `@ryot/client-sdk/react`. Remove stale pre-taxonomy operation or navigation names and any parallel client, capability, routing, reload, lifecycle, or manual request-state facades. Keep `PluginLink` and the reactive location, params, and search hooks as plugin React conveniences backed by the same client/runtime.
...
- **Contract and documentation alignment.** Preserved the exact eight `RyotClientError` reasons, protocol version 1, payload-free lifecycle close, final `data`/`operations`/`navigation` taxonomy, shared Promise and React query/mutation surface, provider-scoped atom registry/cache, query cancellation, one per-session runtime, immutable artifact identities, and revision-bound operation dispatch. Corrected stale protocol wording and architecture text that described theme integration as future work, described atom handling as kernel-only, or required deleting the read-only `crates/` references.
