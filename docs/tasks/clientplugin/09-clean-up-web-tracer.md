...
Preserve the complete demonstrated behavior. This is not a redesign task and must not add features. Keep the production fixture and tests that prove the tracer. Consolidate only verified duplication, remove only code proven unused, and keep package boundaries aligned with the architecture: backend sandbox compiler, client plugin compiler, SDKs, contract/archive, server lifecycle, kernel API/data boundary, route resolver, bridge, and `PluginHost`.
...
- [ ] Exact V1 markers remain simple equality checks without compatibility ranges, fallback protocols, or unused negotiation abstractions.
