...
Extend the shared `@ryot/client-sdk` runtime established by Task 05-followup with `ryot.theme`. The kernel adapter reads the authoritative theme state directly, while the plugin adapter receives a complete initial semantic theme snapshot and subsequent events over its existing `MessageChannel`. The plugin runtime applies those values as CSS variables on its own root document so plugin-local Tailwind classes and client UI SDK components resolve against the same semantic meanings as kernel UI.
...
Theme events use the exact bridge protocol V2. Do not add compatibility negotiation or a fallback bridge.

Theme state must live behind the existing explicit `RyotClient` and `RyotProvider` boundary. Do not add a plugin-only theme singleton, a second React provider, or a bridge API outside the shared client adapter.
...
Task 05-followup defines the runtime seam: extend `createRyotClient`, its explicit React context, and the two environment adapters. The wire event remains an internal detail of the plugin adapter; fixture components consume `ryot.theme`, not `MessagePort` messages.
