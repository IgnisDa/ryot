...
Theme events use strict domain schemas, the exact protocol V3 runtime messages, and the existing session `MessagePort`. Decode the same theme snapshot schema in the direct and bridge adapters and dispatch it through the existing runtime dispatcher. Do not add V2 support, aliases, compatibility negotiation, a fallback bridge, or a theme-specific bridge.
