...
- **Protocol version 1 session.** Strict correlated request/result messages carry the RyotQL document or JSON-compatible public outcome through the single dispatcher, alongside location, theme, and the payload-free `{ type: "lifecycle-close", reason: "disposed" | "failed" }` message. Runtime disposal rejects all remaining plugin query and operation calls exactly once using the shared error contract, while kernel teardown aborts pending service work on a best-effort basis and suppresses late results. An abort cannot undo backend work that committed before it took effect; user, server, plugin, and installation identity remain kernel-owned.
