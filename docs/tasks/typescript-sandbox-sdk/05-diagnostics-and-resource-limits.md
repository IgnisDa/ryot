...
- `sandbox-runtime/limits.ts` owns the fixed production values, UTF-8 measurement, cache/context/request checks, and host-call accounting used by compiler, API, runtime, bridge, HTTP, log, result, and cache boundaries.
- `SandboxCompiler` now starts a one-shot `compiler-worker` Bun process behind a two-permit semaphore. The five-second timeout includes process startup, native TypeScript checking, manifest extraction, bundling, response encoding, and shutdown; cancellation kills the complete process group.
