...
- [x] New `import_run` and `import_run_failure` tables exist with the columns, indexes, cascade behavior, and app-level text enum typing described in the parent PRD.
- [x] A dedicated `import` BullMQ queue and worker are registered in queue setup, worker lifecycle, and runtime startup/shutdown.
- [x] Import routes exist: `POST /imports/runs`, `GET /imports/runs`, `GET /imports/runs/:id`, `GET /imports/runs/:id/failures`, and `DELETE /imports/runs/:id`.
- [x] `POST /imports/runs` validates a discriminated OpenScale input, creates a pending run row, enqueues the job, and returns `{ id }`.
- [x] The worker moves runs through `pending` to `running` to `completed` or `failed`, with `createdAt`, `startedAt`, `finishedAt`, progress, and counters updated according to the parent PRD.
- [x] Source/auth/file-level catastrophic failures mark the run `failed`; item parse failures create failure rows and allow the run to complete.
- [x] Failure rows include user-facing `sourceLabel`/`sourceIdentifier`/`message`/`stage`/`context` suitable for manual recovery without raw file rows, secrets, or temp paths.
