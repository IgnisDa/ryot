...
- [x] `getUserLibraryEntityId(userId)` returns the id of the user's library entity when it exists.
- [x] `getUserLibraryEntityId(userId)` returns `undefined` when no library entity exists for the user.
- [x] `upsertInLibraryRelationship` inserts a row with `relType = 'in_library'`, correct `userId`, `sourceEntityId`, and `targetEntityId`.
- [x] Calling `upsertInLibraryRelationship` twice with the same arguments does not create a duplicate row (idempotent via `onConflictDoNothing`).
- [x] Both functions are exported from their module's `index.ts` barrel.
- [x] Unit tests cover both the happy path and the idempotency of `upsertInLibraryRelationship`.
- [x] `bun run typecheck`, `bun run test`, and `bun run lint` pass in `apps/app-backend`.
