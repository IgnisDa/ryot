...
- [x] `runner-source.txt` parses `driverName` from the stdin payload.
- [x] `runner-source.txt` builds a `driverRegistry` map and injects a `driver(name, fn)`
      function as the first parameter of `sandboxMain`, ahead of the API stubs.
- [x] After the script body executes, the runner calls `driverRegistry[driverName](context)`
      and uses its return value as the result.
- [x] If `driverName` names a driver that was never registered, the runner writes
      `{ success: false, error: "Driver \"<name>\" is not defined in this script" }` to stdout.
- [x] If `driverName` is absent from the payload, the runner falls back to the script's raw
      return value (existing behavior preserved).
- [x] `SandboxEnqueueOptions` in `lib/sandbox/types.ts` gains an optional `driverName` field.
- [x] `lib/sandbox/service.ts` includes `driverName` in the payload sent to the Deno subprocess.
- [x] The `modules/sandbox` enqueue route schema gains an optional `driverName` field.
- [x] `modules/sandbox/service.ts` forwards `driverName` when calling the lib sandbox service.
- [x] A script that registers two drivers and is invoked with each `driverName` in turn returns
      the correct result from each driver.
- [x] Tests covering: driver invoked correctly, unknown driverName error, absent driverName
      fallback.
