...
- [ ] Definition registry entries are keyed by stable plugin identity plus local slug, with an explicit kernel-owned representation.
...
Use stable plugin identity for definition ownership and installation identity for user activation. Do not encode ownership by concatenating slugs into unvalidated strings.
