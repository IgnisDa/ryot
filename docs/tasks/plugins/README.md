...
**Overall Progress:** 7 of 11 tasks completed

**Current Task:** [Task 08](./08-system-plugin-reconciliation-and-conflicts.md) (todo)
...
42. As a user creating a backup, I want my private plugin source packages included, so that the backup is portable to another Ryot server.
43. As a user creating a backup, I want schema-marked plugin secrets redacted, so that the archive does not become a credential bundle.
44. As a user creating a backup, I want shipped plugins recorded as exact requirements rather than copied into the archive, so that trusted server code still comes from the destination installation.
45. As a user restoring a backup, I want private packages validated and compiled before domain records are written, so that restore fails before a partial data import when a package is invalid.
46. As a user restoring a backup, I want private plugin identities remapped consistently, so that entities, events, relationships, integrations, and views retain correct provenance.
47. As a user restoring a backup, I want plugin bootstrap skipped, so that bootstrap-created data is not duplicated alongside archived data.
48. As a user restoring a backup, I want installations with missing redacted secrets marked as needing configuration, so that they cannot execute until they are safe to enable.
49. As a user restoring a backup, I want private installations with complete configuration returned to their archived disabled state after restore, so that restore preserves my intent.
50. As a user restoring a backup, I want archives from the removed format rejected clearly, so that unsupported data is not interpreted ambiguously.
51. As an operator migrating Rust V1 data, I want migrated users to receive media and fitness system installations, so that migrated data resolves through the same model as new accounts.
52. As an operator migrating Rust V1 data, I want provider skeletons to retain trusted system-plugin provenance, so that provider population can reconstruct omitted catalog data.
53. As an operator migrating Rust V1 data, I want migrated integrations attached to each owner's media installation, so that integration ownership is explicit.
54. As an operator migrating Rust V1 data, I want legacy feature preferences applied to built-in saved views, so that migration preserves existing presentation choices without disabling entire plugins.
55. As an operator migrating Rust V1 data, I want plugin bootstrap scripts skipped during migration, so that library and other migrated records are not duplicated.
56. As an operator migrating Rust V1 data, I want unresolved package, provider, schema, or installation mappings to fail migration, so that legacy data is never silently dropped or misattributed.
57. As a Ryot developer, I want package and installation limits enforced before compilation, so that authenticated uploads cannot cause unbounded storage or compiler work.
58. As a Ryot developer, I want plugin ownership represented by database constraints rather than route assumptions, so that background jobs and internal services preserve the same isolation.
...
- Extend the existing process-wide Definition Registry into a qualified package and definition catalog for kernel, system-plugin, and private-plugin definitions.
- The package catalog indexes plugins, manifests, providers, scripts, and qualified definitions by stable plugin ID. Kernel definitions use the explicit kernel-owned representation.
- Effective registry resolution loads current installation state for one user on demand and combines healthy enabled installations for runtime use. Do not add per-user registry caching or cache generations in this plan.
- Definition resolution can include disabled or unavailable installed plugins when decoding qualified historical data.
- Keep the existing process-wide package-catalog invalidation and reconciliation behavior for package ingestion and updates. User installation changes require no cross-process cache invalidation because effective registries are not cached.
- Script, provider, import-source, integration-provider, operation, workflow, automation-binding, saved-view, schema, and signal indexes use qualified plugin identity internally.
...
- Use typed bad-request, unauthorized, not-found, conflict, and sandbox errors consistently with the existing contract.
...
- Shared contract changes and required adaptations to existing app-client consumers are in scope. A new app-client plugin-management interface is not included.
...
- Update product and backup-module documentation to describe only format version 2 and remove version 1 terminology and guarantees.
...
- Adapt the current implemented legacy migration rather than replacing its domain mappings, S3 asset migration, Redis cache migration, reports, or dump-validation runbook.
...
- Preserve current S3 asset ownership and cleanup behavior while rewriting migrated plugin-qualified records through resolved identities. Documented old-object deletion failures remain nonfatal migration warnings.
- Preserve integration-progress and YouTube Music cache migration while resolving required scripts and integrations through final plugin and installation identities.
...
- The scoped plugin registry owns on-demand effective-registry construction, qualified lookups, collision detection, and runtime-versus-definition availability behind one interface.
...
- Process-wide package-catalog tests preserve current cross-process package ingestion and update visibility. User-effective registry tests read current installation state without a per-user cache.
...
- A new app-client plugin-management interface beyond adaptations required by changed existing contracts.
...
- Backend support necessarily changes the shared HTTP contract, existing app-client consumers, and backup fixtures. New plugin-management UI remains out of scope.
