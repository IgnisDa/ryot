...
import { buildLuceneQuery, mbGet } from "../../../lib/vendors/music-brainz";
		...
		return mbGet(host, "artist", {
			...
			Effect.map((dataValue) => {
				...
				const items = (Array.isArray(artists) ? artists : []).flatMap((artist) => {
					...
					return [{ title: name, externalId: id }];
				});
				return {
					items,
					details: {
...
export const details = defineProvider({
	...
	run: (input, host) =>
		Effect.all(
			...
			Effect.map(([artistValue, recordingsValue]) => {
				...
				const releaseGroups = Array.isArray(artist["release-groups"])
