...
- One shared card and row presentation covers every media schema except `show`, `movie`, `music`, `book`, `podcast`, and `video-game`; its loader takes the schema slug from the batch's references.
