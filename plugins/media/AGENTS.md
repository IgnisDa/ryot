...
- One shared card and row presentation covers every media schema except `show`, `anime`, `movie`, `music`, `book`, `manga`, `podcast`, `audiobook`, `comic-book`, `visual-novel`, and `video-game`; its loader takes the schema slug from the batch's references. `person` and `company` keep it and add only a detail page.
- The artwork aspect of a media or creator-credit group schema comes only from `mediaSchemaAspects` in `client/schema-aspects.ts`; its descriptor's header art, recommendation tiles, and creator credit tiles all read it.
- `builtinMediaEntitySchemaSlugs` and `creatorGroupTargetSlugs` live in `shared/media-schema-slugs.ts`.
