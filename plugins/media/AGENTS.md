...
- `MediaDetailBody` frames every descriptor's facts with `mediaRatingFact` first and `mediaProductionStatusFact` last; a descriptor returns only its own facts.
...
- One shared card and row presentation covers every media schema except `show`, `anime`, `movie`, `music`, `book`, `manga`, `podcast`, `audiobook`, `comic-book`, `visual-novel`, and `video-game`; its loader takes the schema slug from the batch's references.
