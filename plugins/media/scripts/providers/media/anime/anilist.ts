...
import { defineProvider, type ProviderDetailsRelatedEntity } from "@ryot/sandbox-sdk/provider";
...
export const search = defineProvider({
	manifest,
	operation: "search",
	run: (input, host) => searchAnilistMedia(host, input, { type: "ANIME", label: "anime" }),
});
...
export const details = defineProvider({
	...
	run: (input, host) => {
		...
		return Effect.gen(function* () {
			...
			const data = yield* anilistGraphql(host, "anime details", MEDIA_DETAILS_QUERY, {
				id: mediaId,
			});
...
export const translate = defineProvider({
	manifest,
	operation: "translate",
	run: (input, host) => translateAnilistMedia(host, input, { type: "ANIME", label: "anime" }),
});
