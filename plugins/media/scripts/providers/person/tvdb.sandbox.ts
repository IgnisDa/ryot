...
export const translate = defineProviderDriver(manifest, "translate", (input, host) => {
	...
	return tvdbGetOptional(host, `/people/${input.externalId}/translations/${providerLanguage}`).pipe(
		Effect.map((translationData) => buildTranslationResult(translationData, null)),
	);
});
