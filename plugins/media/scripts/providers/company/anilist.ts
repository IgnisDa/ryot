...
const collectStudioPages = (
	...
	getStudioPage(host, studioId, page).pipe(
		Effect.flatMap((studioPage) => {
			...
			return Effect.succeed({ studio, mediaEdges: collected.mediaEdges });
		}),
	);
...
export const details = defineProvider({
	...
	run: (input, host) => {
		...
		return collectStudioPages(host, studioId, 1, { studio: null, mediaEdges: [] }).pipe(
			Effect.map(({ studio, mediaEdges }) => {
				...
				const mediaEntities = mediaEdges.flatMap((edge) => {
					...
					if (!media || idValue === null || !providerSlug) {
						return [];
					}
					return [
						{
					...
					relatedEntityGroups: [
						{
							...
							entities: mediaEntities.filter((entity) => entity.providerSlug === "anime.anilist"),
						},
						{
							direction: "outgoing" as const,
							synchronization: "authoritative" as const,
							relationshipSchemaSlug: "company-to-manga",
							entities: mediaEntities.filter((entity) => entity.providerSlug === "manga.anilist"),
						},
					],
				};
			}),
