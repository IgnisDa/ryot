...
describe("adaptNetflixExports", () => {
		...
		const result = Effect.runSync(
			adaptNetflixExports(
				...
				({ title, preferredEntitySchemaSlug }) => {
					...
					return found
						? Effect.succeed({
								matchedTitle: found.sourceLabel,
								entityRef: { kind: "resolved" as const, ...found },
							})
				...
				events: [
					{
						...
						unresolvedEpisode: { type: "show", seasonNumber: 4, episodeNumber: 9 },
					},
					{
	...
	it("records lookup failures and missing episode coverage as item failures", () => {
		const result = Effect.runSync(
			adaptNetflixExports(
				{
					importedAt: "2026-02-10T00:00:00.000Z",
				...
				({ title }) =>
					title.startsWith("The Gentlemen")
						? Effect.succeed({
